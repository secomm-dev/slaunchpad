 ...
 * relationship for the MSI (Inventory) source form. When country = VN, the submitted
 * ward (Magento `city`) MUST belong to the submitted province (Magento `region_id`).
 * An invalid ward is neutralised (cleared) so a malformed ward is never persisted on
 * the source.
 *
...
namespace Secomm\VietNamAddress\Plugin\Inventory;
...
class ValidateVietNamWard
{
    ...
    /**
     * Validate ward(province) relationship for VN before an inventory source is saved.
     *
     * @return array [SourceInterface] (city neutralised if the ward is invalid)
     */
    public function beforeSave(SourceRepositoryInterface $subject, SourceInterface $source): array
    {
        try {
            if ($source->getCountryId() !== 'VN') {
                return [$source];
            }

            $regionId = (string) ($source->getRegionId() ?? '');
            $ward = (string) ($source->getCity() ?? '');
            ...
            if ($collection->getSize() === 0) {
                ...
                $this->logger->warning(
                    'Secomm_VietNamAddress: invalid VN ward for province; neutralising source city.',
            ...
            $this->logger->error(
                'Secomm_VietNamAddress: VN ward validation failed on source save; skipping validation.',
                ['exception' => (string) $e]
            );
        }
