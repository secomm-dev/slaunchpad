 ...
 * dependent ward dropdown (city_select) and a generic 3rd-level sub_city select next to
 * the native city input. The native input is kept as the config VALUE CARRIER so Magento
 * persists the selected ward name into core_config_data unchanged.
 *
 * Country-agnostic + data-driven: ward/sub-city options are loaded by the companion
 * cascade JS (Secomm_AddressDropdown/js/config/address-city) via the generic GraphQL
 * resolvers GetListCity/GetListSubCity (area=adminhtml). VN behaviour/label/validate is
...
class City extends Field
{
     ...
     * city select and a generic sub_city select. The wrapper carries the data-mage-init
     * that drives the generic cascade JS.
     *
    ...
    protected function _getElementHtml(AbstractElement $element): string
    {
        ...
        $this->setData('sub_city_select_id', $cityInputId . '_secomm_subcity_select');
