...
class CurlBuilder
{
    ...
    protected function appendDataToURL()
    {
        ...
        if (count($this->params)) {
            $parameterString .= http_build_query($this->params);
        }

        if ($parameterString) {
            $separator = strpos($this->curlOptions['URL'], '?') !== false ? '&' : '?';
            $this->curlOptions['URL'] .= $separator . $parameterString;
        }

        return $this->curlOptions['URL'];
    }
    ...
    protected function forgeOptions()
    {
        ...
        foreach ($this->curlOptions as $key => $value) {
            ...
            if (!$this->packageOptions['containsFile'] && $key == 'POSTFIELDS' && is_array($value)) {
                if ($this->curlOptions['HTTPHEADER']['0'] == 'Content-Type: application/json') {
                    $results[$arrayKey] = json_encode($value);
                } else {
     ...
     * @return mixed
     */
    public function post($isResend = false)
    {
     ...
     * @return mixed
     */
    public function download($fileName, $isResend = false)
    {
     ...
     * @return mixed
     */
    public function put($isResend = false)
    {
        $this->setPostParameters();
     ...
     * @return mixed
     */
    public function patch($isResend = false)
    {
        $this->setPostParameters();
     ...
     * @return mixed
     */
    public function delete($isResend = false)
    {
        $this->appendDataToURL();
        if ($isResend) {
     ...
     * @return $this
     */
    protected function withUserPwd($user, $pw)
    {
        $this->curlOptions['USERPWD'] = "$user:$pw";
        return $this;
    }
