...
class ProductList
{
    ...
    public function getProductCollection($source = 'latest', $numberItems = 8, $order = 'newestfirst', $conditions = '', $storeId = \Magento\Store\Model\Store::DEFAULT_STORE_ID, $isShowOutOfStock = true)
    {
        ...
        $visibleIds = $this->catalogProductVisibility->getVisibleInCatalogIds();
        $collection->addAttributeToFilter('visibility', $visibleIds, 'left')
            ->getSelect()
            ->having('visibility IN (?)', $visibleIds);
