	...
	window.MgzMediabrowserUtility = {
		...
		openDialog: function(url, width, height, title, options) {
			...
			if (options && options.targetElementId) {
				...
				if (window.MediabrowserUtility) {
					window.MediabrowserUtility.targetElementId = options.targetElementId;
				}
			}

			var isNewGallery = url.indexOf('media_gallery') !== -1;

			if (isNewGallery && this.modal && this.modal.length > 0 && this.modal.html().length > 0) {
            ...
            if (!this.modal || this.modal.length === 0) {
                var content = '<div class="popup-window magento-message" id="' + windowId + '"></div>';
                this.modal = $(content).modal($.extend({
                    title: title || 'Insert File...',
                    modalClass: 'magento magento-media-browser',
                    type: 'slide',
                    buttons: []
                }, options));
            }

            require(['mage/adminhtml/browser'], function(nativeBrowser) {
                nativeBrowser.modal = self.modal;
            });
	...
	$.widget("mage.mgzmediabrowser", $.mage.mediabrowser, {
		...
		insert: function(event) {
			...
			return $.ajax({
				...
				const evt = new CustomEvent("input:update", { detail: data });
				targetEl[0].dispatchEvent?.(evt);
			}, this));
		},
