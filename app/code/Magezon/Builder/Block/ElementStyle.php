...
class ElementStyle extends \Magento\Framework\View\Element\Template
{
    public function getStylesHtml()
    {
        ...
        if ($element->getData('animation_duration')) {
            $styles['animation-duration'] = $element->getData('animation_duration') . 's';
        }
        ...
        if ($html) {
            $html = '<style class="mgz-style">' . $html . '</style>';
        }
        return $html;
    }
    ...
    public function getLineStyles()
    {
        ...
        if ($element->getData('show_line')) {
            ...
            $stylesHtml = $this->parseStyles($styles);
            if ($stylesHtml) {
                $html .= $htmlId . ' .mgz-elem-heading-line:before{';
                $html .= $stylesHtml;
                $html .= '}';
            }
        }
    ...
    public function getTabsStyleHtml()
    {
        ...
        } elseif ($element->getData('tab_border_width') != '') {
            $styles['margin'] = '0';
        }
