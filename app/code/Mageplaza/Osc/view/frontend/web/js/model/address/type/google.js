        ...
        return Class.extend(
            {
                ...
                initAddressElements: function (fieldsetName) {
                    ...
                    $.each(
                        elementFields, function (index, field) {
                            registry.async(fieldsetName + '.' + field)(
                                function (fieldElement) {
                                    if (field === 'street') {
                                        $.each(
                                            fieldElement.elems(), function (key, elem) {
                                                if (key === 0) {
                                                    fieldElement           = elem;
                ...
                fillInAddress: function (components) {
                    ...
                    $.each(
                        this.addressElements, function (index, element) {
                            if (element.visible() && components.hasOwnProperty(index)) {
                                if (index === 'region_id') {
                                    $.each(
                                        element.options(), function (key, option) {
                    ...
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                            function (position) {
                                ...
                                geocoder.geocode(
                                    {'latLng': location}, function (results, status) {
