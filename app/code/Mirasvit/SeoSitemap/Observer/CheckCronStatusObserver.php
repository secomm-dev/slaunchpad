    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->request
            ...
            $cronStatus = $this->cronHelper->checkCronStatus(
                "",
                false,
                (string)__('A cron job is required for automatic sitemap generation. Automatic generation can be configured at System > Configuration > Catalog > Google Sitemap > Generation Settings. Cron for Magento is not running. To set up a cron job, follow the link.')
            );
            if ($cronStatus !== true) {
                //@fixme
