...
class Save extends \Mirasvit\SeoAutolink\Controller\Adminhtml\Link
{
    ...
    public function execute(): void
    {
        ...
        if ($data = $this->getRequest()->getParams()) {
            $linkType = $data['link_type'] ?? \Mirasvit\SeoAutolink\Api\Data\LinkInterface::LINK_TYPE_URL;

            if ($linkType === \Mirasvit\SeoAutolink\Api\Data\LinkInterface::LINK_TYPE_URL) {
                $data['url'] = trim($data['url'] ?? '');
                if ($data['url'] === '') {
                    $this->messageManager->addError((string)__('URL is required for custom URL links.'));
                ...
                if (empty($data['entity_id'])) {
                    $this->messageManager->addError((string)__('Entity ID is required for product/category links.'));
                    $this->backendSession->setFormData($data);
                    $this->_redirect('*/*/edit', ['id' => $this->getRequest()->getParam('id')]);
                    return;
                }
