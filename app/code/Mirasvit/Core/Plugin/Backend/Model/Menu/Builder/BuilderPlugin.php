        ...
        foreach ($this->packageService->getPackageList() as $package) {
            foreach ($package->getModuleList() as $moduleName) {
                ...
                foreach ($items as $idx => $item) {
                    ...
                    if (($this->config->isCustomAdminUrl() && strpos($action, 'system_config') === false) || (strpos($action, $areaFrontName) === 0 && strpos($action, 'sitemap') === false && strpos($action, 'system_config') === false)) {
