...
class YamlService
{
    ...
    private function _toType($value)
    {
        ...
        if ($is_quoted) {
            if ($first_character == "'") {
                // Single-quoted scalars are literal (YAML spec): no backslash
                // escapes are processed, only '' -> '. In particular a literal
                // "\n" must stay two characters, so a JSON blob that dump()
                // wrapped in single quotes survives the round-trip intact —
                // dumping the raw blob then re-parsing it no longer corrupts the
                // escaped newlines into real ones (mirasvit/pm#254).
                return strtr(substr($value, 1, -1), ['\'\'' => '\'', '\\\'' => '\'']);
            }
            // Double-quoted scalars DO process escapes (\n -> newline, \" -> ").
            $value = str_replace('\n', "\n", $value);
            return strtr(substr($value, 1, -1), ['\\"' => '"', '\\\'' => '\'']);
        }
