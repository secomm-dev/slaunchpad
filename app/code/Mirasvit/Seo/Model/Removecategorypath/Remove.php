...
class Remove extends \Magento\Framework\DataObject
{
    ...
    public function removeParentCategoryPath()
    {
        ...
        if (!$this->storeIds) {
            return (string)__('Something went wrong.');
        }
        ...
        foreach ($urlRewriteCategory as $categoryRewrite) {
            ...
            if ($this->table
                ...
                if ($update) {
                    $this->insertCategoryUrlRewriteRedirect();
                    $this->updateCategoryUrlRewriteRedirect();
                }
            }
        }

        return (string)__('Parent category path has been removed.');
    }
    ...
    {
        $categoryId = null; // set a category ID here to limit processing to that category
        if ($categoryId !== null) {
    ...
    /**
     * Update data in url_rewrite table of processed category
     *
     * @return bool
     */
    ...
    protected function insertCategoryUrlRewriteRedirect(): bool
    {
        $bind = [
            'entity_type' => self::ENTITY_TYPE,
            'entity_id' => $this->categoryId,
    ...
    protected function updateCategoryUrlRewriteRedirect(): bool
    {
        $select = $this->connection->select()
            ->from($this->table, 'url_rewrite_id')
            ->where("'" . self::ENTITY_TYPE . "'" . ' = entity_type')
