...
class Redirect extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getUrlWithCorrectEndSlash($url)
    {
        ...
        $url = preg_replace('/\\/+/', '/', $url) ?? $url; //delete multi slash
        ...
        $rchr = strrchr($url, '.');
        $extension = $rchr !== false ? substr($rchr, 1) : '';
    ...
    public function redirect($response, $url, $redirectType = 301)
    {
        ...
        $this->actionFlag->set('', \Magento\Framework\App\Action\Action::FLAG_NO_DISPATCH, '1');
