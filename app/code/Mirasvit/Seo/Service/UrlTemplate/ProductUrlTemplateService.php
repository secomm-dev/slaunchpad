...
class ProductUrlTemplateService
{
    ...
    {
        return $message . ':: ' . __('Product ID: %1 Store ID: %2 Url Key: %3', $pid, $sid, $key);
    }
    ...
    public function processUrlRewriteCollection(
        ...
        foreach ($urlRewriteCollection as $rewrite) {
            ...
            if (isset($inputStoreId) && $inputStoreId != $storeId) {
                continue;
            }
            ...
            if ($urlKey === $product->getUrlKey()) {
                yield $this->message((string)__("Already used url key"), $productId, $storeId, $urlKey);
             ...
             */
            if (!$urlKey) {
                yield $this->message((string)__("Empty url key"), $productId, $storeId, $urlKey);
            } elseif (!$isUnique) {
                yield $this->message((string)__("Duplicate used url key"), $productId, $storeId, $urlKey);
            } else {
                ...
                yield $this->message((string)__("Updated url key"), $productId, $storeId, $urlKey);
            }
        }
    }
    ...
    public function processProductCollection(
        ...
        $productCollection->setPageSize(2000);
    ...
    private function applyUrlKey(string $urlKey, ProductInterface $product): void
    {
        ...
        if ($product->isVisibleInSiteVisibility() || $product->getStoreId() === Store::DEFAULT_STORE_ID) {
            ...
            if ($productUrlRewriteGenerator !== null) {
