...
class DataProvider extends \Magento\Framework\View\Element\UiComponent\DataProvider\DataProvider
{
    ...
    protected function searchResultToOutput(SearchResultInterface $searchResult): array
    {
        ...
        $childQuery = "SELECT main_table.url_id as url_id, GROUP_CONCAT(t2.url_id) as linked
                    ...
                    LEFT JOIN {$resource->getTable(UrlInterface::PARENT_TABLE_NAME)} as parent_rel
                    ON parent_rel.parent_url_id = main_table.url_id
                    LEFT JOIN {$resource->getTable(UrlInterface::TABLE_NAME)} as t2
                    ON t2.url_id = parent_rel.url_id AND t2.url_id != main_table.url_id AND t2.job_id >= {$jobId}
                    WHERE main_table.url_id IN ({$urlIds}) GROUP BY main_table.url_id";
