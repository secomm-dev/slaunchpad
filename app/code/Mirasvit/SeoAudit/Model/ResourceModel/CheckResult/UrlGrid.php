...
class UrlGrid extends SearchResult
{
    ...
    {
        if (is_string($field) && in_array($field, [CheckResultInterface::JOB_ID, CheckResultInterface::IDENTIFIER, CheckResultInterface::RESULT])) {
            $field = 'main_table.' . $field;
        }

        if ($field == 'main_table.' . CheckResultInterface::RESULT && is_array($condition)) {
            $value = $condition['eq'] ?? null;
            $range = [];
