...
class UrlService
{
    ...
    public function loadResource(UrlInterface $url): UrlInterface
    {
        ...
        // No curl_close(): since PHP 8.0 the handle is a CurlHandle object released by refcount
        // (the call is a documented no-op, and deprecated as of 8.5); on 7.4 the resource is freed
        // when $ch goes out of scope at the end of this method.
    ...
    public function ensureUrl(string $url, string $type, int $jobId, ?int $parentId = null): ?UrlInterface
    {
        ...
        if (!$url
            || preg_match('/(mailto|tel|javascript):/is', $url)
            || preg_match('#/cdn-cgi/#i', $url)
            || $this->isAdminAreaUrl($url)
            || $this->isExcluded($url)
        ) {
            return null;
        }
    ...
    private function normalizeUrl(string $url): ?string
    {
        ...
        $url = preg_replace('@[a-z0-9\%]{48,}@is', '#hashkey', $url) ?? $url;
