# Implementation Plan: SL-021 — Data model maximum_discount_amount (column + extension attribute)

| Field | Value |
|---|---|
| Specification | tickets/SL-021-maxdiscount-db-schema.md (`## Mini Spec`, embedded — ID SL-021) · canonical parent specs/SPEC-FEAT-008-promotion-max-discount.md (FULL, VALID) §6, §14 |
| Author | AI draft |
...
|---|---|---|
| Column spec? | `maximum_discount_amount` DECIMAL(12,4) UNSIGNED NULL default NULL trên bảng core `salesrule`; semantic NULL/0 = unlimited, >0 = cap base-currency (by_percent) | DEC-FEAT008-001 §3, spec §6 |
...
| Extension attribute Phase 1? | Có (D2 resolved) — `etc/extension_attributes.xml` cho `Magento\SalesRule\Api\Data\RuleInterface`, attr `maximum_discount_amount` type `float` | DEC-FEAT008-001 §3 |
...
- **i18n:** slice này không thêm string UI (admin field thuộc SL-023) — không đổi `i18n/*.csv`.
...
### 1.4 Rủi ro môi trường (lesson SL-020)
...
| 10 | Records: evidence `.ai/runtime/evidence/SL-021/SL-021-evidence.md` (pattern SL-020) · ticket AC tick + status + AI Pre-review §8.3 · plan status update | `.ai/` | — | — |
...
Task 8–9 (AC-4 rollback doc + AC-5 declarative-only sweep)
   ↓
AI Pre-review §8.3  →  TL code review (Tier-2)  ⏳ HUMAN  →  QC trên DB snapshot (SL-024/025 scope)
```

Rollback nếu sai giữa chừng: column additive, chưa có consumer (SL-022 mới đọc) → disable module là đủ để vô hiệu hoá; drop column thủ công chỉ khi cần sạch schema (README document).

## Remaining steps (human / next tickets)

1. **TL Tier-2 approval** — duyệt plan này + schema change (nói "approve SL-021" trong chat là đủ theo precedent DEC-FEAT008-001).
2. Sau merge: SL-022 (cap engine) consume field; SL-023 (admin UI) dựng form field trên data layer này.
3. QC trên DB snapshot trước merge (ticket Risks) — thực hiện cùng SL-024 integration tests.
...
| Expected Behavior — totals/pricing không đổi | Column inert: không collector/engine đọc (SL-022 mới consume) — `generated/metadata` diff chỉ thêm plugin wiring RuleRepository, không total collector |
| Constraints — thuần declarative, không vendor change | Task 9 (AC-5) |
| Constraints — semantic NULL/0 unlimited | db_schema default NULL + README semantic note (engine guard thuộc SL-022) |
