# TASK-NDASAD Implementation Plan — Secomm ShippingCore origin contract + GHTK origin refactor

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-NDASAD-shippingcore-origin-contract.md |

> **Mode A** · Tier 2 · Status: **Approved 2026-08-17 (user acting as SA/TL; DEC-TASKNDASAD-001 accepted) — Phase C implement cleared**
...
**`Secomm_Ghtk`** (TASK-YJENM2 + TASK-BRKHN4, vừa build, **rate-only** — order sync TASK-KV328X parked):
...
collectRates(RateRequest)                         Model/Carrier/Ghtk.php
 ...
 │   └─ secomm_ghtk_address_map (TASK-YJENM2) → miss → WardIdBridge → best-effort vi_VN (DEC-TASKYJENM2-001)
 ...
 │   └─ strict gate DEC-TASKBRKHN4-001: thiếu → null → carrier hide
 ├─ ShipmentWeightCalculator → grams (DEC-TASKBRKHN4-002)
...
- Không có shipment/order flow, tracking, cancel, label (TASK-KV328X parked, DEC-TASKKV328X-001/024 pending).
...
- `Secomm_Ghtk` mapping table `secomm_ghtk_address_map` key `(country_id, region_id, ward_id)` (DEC-TASKYJENM2-001); ward_id = `directory_region_city.city_id` (VN 2-level, ward = native city).
- TASK-8WSERX/TASK-2V0AEV đã đưa VN ward dropdown vào admin **Shipping Origin** + **Store Information** + **MSI Source** form (ward persist vào native `city`, region_id vào `region_id`; `ValidateVietNamWard` plugin validate khi save). → Dữ liệu Shipping Origin / MSI Source **đã ở dạng VN 2-level** — điều kiện sẵn cho default provider + future fulfillment provider.
...
| P7 | **TASK-KV328X (order sync) plan reuse `PickupAddressResolver`** (AC-6) — nếu không có abstraction chung, rate và order sync có nguy cơ lệch origin source | TASK-KV328X AC-6 |
...
**Đặt deliberately carrier-local (lean, không tạo core abstraction):** request mapper interface (chỉ 1 carrier tồn tại; payload shape khác nhau từng carrier), capability interfaces (GHTK rate-only — tạo khi TASK-KV328X resume), common result DTO (Magento `Rate\Result`/`Method` đã đủ — reuse), response contracts (`FeeResult` carrier-local).
...
// không trả null, không quyết validity — carrier tự đánh giá usable (ISP/DEC-TASKBRKHN4-001)
```

- Provider default đọc `shipping/origin/*` (country_id, region_id, city→ward, street_line1/2→street, postcode; `telephone/contactName/district = null` — shipping origin không có các field này; decorator/enrichment là việc của module khác). Province name = region default name (Magento_Directory); **regionId giữ nguyên** để carrier normalize sang tên GHTK bằng id (DEC-TASKYJENM2-001 canonical key).
- Metadata dotted-key `{carrierCode}.{key}` — flat, không nested array, không hard-code GHTK/GHN field trong core.
- Ward **id** không đưa vào Origin (YAGNI — không surface nào persist ward id trên address/source hiện tại; carrier bridge bằng (regionId, wardName) như DEC-TASKYJENM2-001 path B).

**Key decisions (→ DEC-TASKNDASAD-001):**
1. Origin contract như trên; provider không quyết validity (carrier-side gate giữ DEC-TASKBRKHN4-001).
2. BC chain: legacy `pick_*` (bất kỳ field set) thắng; chỉ khi **tất cả trống** mới delegate Shipping Origin. Half-filled legacy KHÔNG fallback (tránh ship từ kho sai — giữ strict DEC-TASKBRKHN4-001).
...
4. Không tạo: core mapper interfaces, capability interfaces, common result DTO (lean — tạo khi có carrier thứ 2 / TASK-KV328X resume).
...
- Semantic mới: `pick_*` toàn trống → origin = Magento Shipping Origin (trước đây: carrier inactive). Đây là behavior change có chủ đích (AC-11 TASK-NDASAD).
...
| Carrier behavior | Merchant đã set pick_*: **không đổi** (legacy wins, DEC-TASKBRKHN4-001 strict giữ). Merchant chưa set: trước đây inactive → giờ dùng Shipping Origin (muốn vậy) |
...
- **Goal:** strict gate DEC-TASKBRKHN4-001 trên Origin; normalize regionId-based origin sang GHTK names.
...
- **Goal:** tách payload mapping khỏi client; carrier dùng provider chain; cùng origin abstraction cho rate (và TASK-KV328X sau này).
...
- **Risk:** cao nhất trong plan — chạm rate path; mitigations: giữ try/catch resilient (AC-011 TASK-BRKHN4), không đổi param gửi GHTK.
...
- **Acceptance:** `vendor/bin/phpunit -c dev/tests/unit/phpunit.xml.dist --filter 'Secomm\\(ShippingCore|Ghtk)'` green; evidence ghi `.ai/runtime/evidence/TASK-NDASAD/`.
- **Risk:** thấp.

### Task 9 — Documentation + records

- **Goal:** đóng record theo AGENTS §14.
- **Files:** ShippingCore/Ghtk `README.md` + `CHANGELOG.md`; DEC-TASKNDASAD-001 proposed→(TL approve); TASK-NDASAD status update; FEAT-AE761Z note (origin resolution chuyển sang ShippingCore — không đổi fee scope); project-context 04/09/10 diff (surface cho human commit).
...
```
[TL/SA approve DEC-TASKNDASAD-001 + plan này]
...
Không implement bất kỳ task nào trước khi DEC-TASKNDASAD-001 + plan được approve (AGENTS §7.1, §8.2, §10).
