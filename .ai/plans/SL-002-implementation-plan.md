...
| Depends on | [SL-001](SL-001-implementation-plan.md) (module expose Magewire address component + GraphQL) |
...
**Strategy B (DEC-7):** reuse Magewire address cascade component từ SL-001; wire vào OSC address region (shipping + billing).
...
| Project integration code (theme-level Magewire/.phtml) | new | Wire SL-001 component → OSC; sync address field với OSC submit (AC-002) |
...
0. **OSC seam research (state 3, mandatory)** — risk: high — deps: SL-001 done
...
1. **Theme override scaffold** — risk: medium — deps: step 0, SL-001
...
2. **Shipping address integration (AC-002)** — risk: high — deps: step 1
   - Nhúng Magewire address cascade (SL-001) vào OSC shipping form; sync `region_id`/`city`/`sub_city` với OSC shipping address model.
...
| SL-001 interface chưa đủ → wire khó | medium | Chốt interface contract giữa SL-001/SL-002 sớm |
...
- Module `Secomm_AddressDropdown` changes (→ SL-001, DEC-8).
...
- **SL-001 interface contract**: module expose gì (Magewire component public API + GraphQL) để Launchpad wire? (chốt khi làm SL-001)
- **Gate**: `plan-approval` (TL, Level 2) — **sau khi** step 0 research xong + plan cụ thể hoá.
- **Sequencing**: SL-002 chỉ bắt đầu **sau** SL-001 hoàn thành (hoặc chốt interface sớm).
