# Implementation Plan: TASK-FD6A9X — VN 2-level Cart "Estimate Shipping and Tax" (Luma + Hyvä)

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-FD6A9X-vn-cart-shipping-estimate.md |
...
> Parent: [FEAT-JSZQV3](../records/features/FEAT-JSZQV3.md). Spec: [vn-cart-shipping-estimate](../specs/SPEC-TASK-FD6A9X-vn-cart-shipping-estimate.md).

## Metadata

| Field | Value |
|-------|-------|
| Ticket | [TASK-FD6A9X](../tickets/TASK-FD6A9X-hyva-cart-estimate-city-cascade.md) |
| Spec | [vn-cart-shipping-estimate](../specs/SPEC-TASK-FD6A9X-vn-cart-shipping-estimate.md) (Draft) |
| Feature | [FEAT-JSZQV3](../records/features/FEAT-JSZQV3.md) |
| Author | AI draft |
| Reviewer (TL) | user (acting as SA/TL) — approved 2026-07-29 |
| Workflow Mode | A |
| Date | 2026-07-29 |
| Decisions | DEC-FEATJSZQV3-001 · DEC-FEATJSZQV3-002 · DEC-FEATJSZQV3-003 · DEC-8 |
| Validation level | **L3** (address data — §8.6) |

## 1. Approach

**Reuse-first, VN-owned (DEC-FEATJSZQV3-001/019):** mở rộng `Secomm_VietNamAddress` (data-only → +`view/frontend`) để owns hành vi VN; **reuse** `Secomm_AddressDropdown` (data/GraphQL `GetListCity`/`Helper`/cart rate plugin) — **không sửa generic** (cleanup leak VN = TASK-KCBDDT riêng).
...
| **Không sửa** `Secomm_AddressDropdown/*` | — | generic country-agnostic (DEC-FEATJSZQV3-003); leak → TASK-KCBDDT |
...
6. **QC + evidence** — risk: low — deps: 5
   - 9-matrix QC; evidence `.ai/runtime/evidence/FEAT-JSZQV3/` (TASK-FD6A9X portion).
...
## 6. Out of scope
- District; carrier code mapping (SL-004/005/006); OSC; generic VN-leak cleanup (TASK-KCBDDT); schema/endpoint/storage mới; resolver hardening (Q1); destructive district migration.
