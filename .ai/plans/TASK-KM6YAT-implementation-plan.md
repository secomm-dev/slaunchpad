# TASK-KM6YAT Implementation Plan — GHTK order submit qua Magento native shipping-label flow

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-KM6YAT-ghtk-native-label-flow.md |

> **Mode A** · Tier 2 · Status: **Approved 2026-08-17 (user acting as SA/TL; DEC-TASKKM6YAT-001 accepted) — Phase C cleared**
...
- Tracking/label: chưa lưu ở đâu; COD: chưa có logic nào (TASK-KV328X parked; COD payment = BA backlog).
...
| COD/origin chưa có | `CodAmountResolverInterface` + origin chain TASK-NDASAD (`fromShipment()`) |
...
- Risks: Q-EXT order API contract chưa verify; container popup UI cần QC thật; sync call trong admin request (timeout 5s); DEC-TASKKV328X-001 supersession cần approval.
...
- **Goal:** context cho shipment path (AC-6; hoàn tất AC-10 TASK-NDASAD).
...
- **Files:** tests ở Tasks 2-5 + tổng run; CHANGELOG/README Ghtk (1.2.0); TASK-KV328X ticket note (redirect → TASK-KM6YAT); TASK-KM6YAT ticket AC check; evidence `.ai/runtime/evidence/TASK-KM6YAT/`.
- **AC:** phpunit suite green (84+ mới); DI compile OK; validator records OK. **Risk:** thấp.

### Sequence & gating

```
[SA/TL approve DEC-TASKKM6YAT-001 (supersedes DEC-TASKKV328X-001)]
