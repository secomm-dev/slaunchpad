# Implementation Plan: TASK-4ZV5NG — Admin VN 2-level address dropdown (customer-address form)

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-4ZV5NG-admin-vn-address-customer-form.md |
...
> Parent: [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md). Ticket: [TASK-4ZV5NG](../tickets/TASK-4ZV5NG-apply-address-dropdown-admin-customer-form.md). Spec: [admin-vn-address-customer-form](../specs/SPEC-TASK-4ZV5NG-admin-vn-address-customer-form.md) — **approved 2026-08-03 (user acting as SA/TL)**.

## Metadata

| Field | Value |
|-------|-------|
| Ticket | [TASK-4ZV5NG](../tickets/TASK-4ZV5NG-apply-address-dropdown-admin-customer-form.md) |
| Spec | [admin-vn-address-customer-form](../specs/SPEC-TASK-4ZV5NG-admin-vn-address-customer-form.md) — **approved** |
...
**Tách generic mechanism (AddressDropdown) vs country adapter (VietNamAddress) — DEC-FEATJSZQV3-003/025.** Mirror frontend cart precedent (TASK-FD6A9X `shipping.phtml` + `ValidateVietNamWard`): VN behaviour gated `country == 'VN'`, ward = native `city`, 2-level (không `sub_city`), server-side validate ward ∈ province.
...
- **`Secomm_VietNamAddress`** (NEW admin adapter): khi `country == 'VN'` → overlay label `city_select` = "Phường/Xã", **hide `sub_city`** (2-level DEC-TASKYJENM2-001/025), reuse data `GetListCity(region_id)` (KHÔNG duplicate, DEC-FEATJSZQV3-003), ward persist native `city`. Non-VN → generic mechanism nguyên vẹn.
...
- **Persist**: `customer_address_entity.city` (ward name string, VN — tech-debt DEC-TASKYJENM2-001, path A = future); `sub_city` giữ generic cho country non-VN 3-level. Không cross-wire store, không đổi `db_schema`.

**Lý do chọn:** tách generic/adapter đúng DEC-FEATJSZQV3-003/025 (TASK-KCBDDT align ở storefront); reuse data read-only; validate mirror precedent đã proven; sửa gap Add-new (bug hiện tại) trong cùng pass.
...
| **KHÔNG sửa** `Secomm_AddressDropdown/view/frontend/*`, GraphQL resolvers, `db_schema.xml`, `Selector\City`/`SubCity` source models | — | generic country-agnostic (DEC-FEATJSZQV3-003); reuse read-only; `sub_city` column giữ |
...
6. **Tests + QC + evidence (L3)** — risk: low — deps: 2,3,4,5
   ...
   - evidence → `.ai/runtime/evidence/FEAT-E2HM1J/` (TASK-4ZV5NG).

## 4. Regression risks

| Risk | Severity | Mitigation |
|------|----------|------------|
| Migrate break storefront (customer form/cart/OSC dùng cascade) | high | không chạm `view/frontend/*`; TASK-KCBDDT coordinate; regression AC-9 QC full |
...
- ward_id / `city_id` canonical persistence (path A, DEC-TASKYJENM2-001 long-term) — ward vẫn persist name string.
...
**Fix (Option A — hardening, blast radius nhỏ, giữ generic mechanism theo DEC-FEATE2HM1J-001):**
...
4. **(kèm) GraphQL escape** `escapeGraphQlValue` cho `region_id`/`default_name` (mirror TASK-SQY42T `address-cascade.js`; rule "escape all output").
