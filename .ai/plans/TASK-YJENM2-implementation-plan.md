# Implementation Plan: TASK-YJENM2 — GHTK address mapping

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-YJENM2-ghtk-address-mapping.md |
...
> Parent: [FEAT-AE761Z](../records/features/FEAT-AE761Z.md). Ticket: [TASK-YJENM2](../tickets/TASK-YJENM2-ghtk-address-mapping.md). Spec: [ghtk-address-mapping](../specs/SPEC-TASK-YJENM2-ghtk-address-mapping.md).

## Metadata

| Field | Value |
|-------|-------|
| Ticket | [TASK-YJENM2](../tickets/TASK-YJENM2-ghtk-address-mapping.md) |
| Spec | [ghtk-address-mapping](../specs/SPEC-TASK-YJENM2-ghtk-address-mapping.md) |
| Feature | [FEAT-AE761Z](../records/features/FEAT-AE761Z.md) |
| Author | AI draft |
| Reviewer (TL) | user (acting as SA/TL) — **pending approval** |
| Workflow Mode | A |
| Date | 2026-07-30 |
| Decisions | DEC-TASKYJENM2-001 (accepted, path B) · DEC-TASKBRKHN4-001 (accepted) · DEC-FEATJSZQV3-002 · DEC-FEATJSZQV3-003 · DEC-8 |
| Validation level | **L3** (address data + admin upload — §8.6) |

## 1. Approach

**Carrier-owned mapping, reuse data (DEC-FEATJSZQV3-002/019):** bảng `secomm_ghtk_address_map` + `DestinationAddressResolver` nằm trong `Secomm_Ghtk`; **reuse** `Secomm_AddressDropdown`/`VietNamAddress` data (read-only, không sửa generic).

- **Canonical key `(country_id, region_id, ward_id)`** trên `city_id` (DEC-TASKYJENM2-001; VN 2-level → ward = city level). **Path B bridge**: legacy payload (chỉ `ward_name`) → resolve `city_id` từ `directory_region_city` deterministic theo `(region_id, default_name)` (city table có sẵn region_id). Không expose/persist `city_id` ở generic (path A = long-term).
- **Best-effort fallback** (DEC-TASKYJENM2-001): mapping miss → vi_VN names từ data layer; **không assert "API luôn work"** — GHTK có thể reject; caller (TASK-BRKHN4) xử lý graceful.
...
**Lý do chọn:** path B = lean, tôn trọng DEC-FEATJSZQV3-003/DEC-8 (không sửa generic); best-effort fallback chính xác → không che gap; importer transactional → không corrupt bảng.
...
| **Không sửa** `Secomm_AddressDropdown/*`, `Secomm_VietNamAddress/*` | — | generic country-agnostic (DEC-FEATJSZQV3-003); reuse read-only |
...
| Fallback che gap → kỳ vọng "luôn work" | medium | log masked; QC test miss = best-effort (có thể no-rate ở TASK-BRKHN4) |
