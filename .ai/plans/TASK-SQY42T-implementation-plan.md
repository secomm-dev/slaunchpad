# Kế hoạch triển khai: TASK-SQY42T — Apply VN 2-level address dropdown trên admin order form

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-SQY42T-admin-vn-address-order-form.md |
...
> Parent: [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md). Ticket: [TASK-SQY42T](../tickets/TASK-SQY42T-apply-address-dropdown-admin-order-form.md). Spec: [admin-vn-address-order-form](../specs/SPEC-TASK-SQY42T-admin-vn-address-order-form.md).

## Metadata

| Field | Value |
|-------|-------|
| Ticket | [TASK-SQY42T](../tickets/TASK-SQY42T-apply-address-dropdown-admin-order-form.md) |
| Spec | [admin-vn-address-order-form](../specs/SPEC-TASK-SQY42T-admin-vn-address-order-form.md) |
| Feature | [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md) |
| Author | AI draft |
| Reviewer (TL) | user (acting as SA/TL) — pending approval |
| Workflow Mode | A |
| Date | 2026-08-03 |
| Decisions | DEC-FEATE2HM1J-001 · DEC-TASKYJENM2-001 · DEC-FEATJSZQV3-003 · DEC-17 |
| Validation level | **L3** (order + PII + admin save path) |

## 1. Hướng tiếp cận

**Reuse-first, admin-order-owned wiring.** Mình sẽ giữ `Secomm_AddressDropdown` ở vai trò generic data/mechanism và đẩy behavior VN vào `Secomm_VietNamAddress` theo DEC-FEATJSZQV3-003/025. Với admin order form, không port trực tiếp provider-mixin của customer modal vì surface này là `Magento_Sales` block-based + `window.order` bootstrap, không phải `Magento_Ui/js/form/provider`.
...
- Ghi evidence vào `.ai/runtime/evidence/FEAT-E2HM1J/` hoặc folder tương ứng của TASK-SQY42T.
...
- Store Information + Shipping Origin (`TASK-8WSERX`).
- MSI Source (`TASK-2V0AEV`).
...
**Cơ chế THẬT** (generic, đúng DEC-FEATJSZQV3-003) cho city/ward trên admin order form = `Secomm_AddressDropdown`:
