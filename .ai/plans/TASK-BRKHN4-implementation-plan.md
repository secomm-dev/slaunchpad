# Implementation Plan: TASK-BRKHN4 — GHTK carrier + rate calculation

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-BRKHN4-ghtk-carrier-rate.md |
...
> Parent: [FEAT-AE761Z](../records/features/FEAT-AE761Z.md). Ticket: [TASK-BRKHN4](../tickets/TASK-BRKHN4-ghtk-carrier-rate.md). Spec: [ghtk-carrier-rate](../specs/SPEC-TASK-BRKHN4-ghtk-carrier-rate.md).

## Metadata

| Field | Value |
|-------|-------|
| Ticket | [TASK-BRKHN4](../tickets/TASK-BRKHN4-ghtk-carrier-rate.md) |
| Spec | [ghtk-carrier-rate](../specs/SPEC-TASK-BRKHN4-ghtk-carrier-rate.md) |
| Feature | [FEAT-AE761Z](../records/features/FEAT-AE761Z.md) |
| Author | AI draft |
| Reviewer (TL) | user (acting as SA/TL) — **pending approval** |
| Workflow Mode | A |
| Date | 2026-07-30 |
| Decisions | DEC-TASKBRKHN4-001 (accepted) · DEC-TASKBRKHN4-002 (accepted, multi-select) · DEC-FEATJSZQV3-002/019/DEC-8 |
| Validation level | **L3** (shipping carrier + secret + checkout-critical — §8.6) |
| Depends on | [TASK-YJENM2](TASK-YJENM2-implementation-plan.md) (DestinationAddressResolver + mapping) |

## 1. Approach

**Carrier-owned rate logic, reuse TASK-YJENM2 resolver (DEC-FEATJSZQV3-002/019):** `Secomm_Ghtk` owns carrier + rate; reuse `DestinationAddressResolver` (TASK-YJENM2). KHÔNG sửa generic.
...
| `Secomm/Ghtk/etc/adminhtml/system.xml` + `acl.xml` | new/modify | config fields + ACL (AC-002/005) — append vào TASK-YJENM2 system.xml |
...
| **Reuse** `DestinationAddressResolver` (TASK-YJENM2) | — | inject vào carrier (AC-001 flow) |
| **Không sửa** generic modules | — | DEC-FEATJSZQV3-003 |

## 3. Steps (độc lập reviewable, theo thứ tự)

1. **Carrier skeleton + config + i18n** — risk: low — deps: TASK-YJENM2
...
- Order sync / GHTK order / webhook / `pick_money` / COD (TASK-KV328X, PARKED).
