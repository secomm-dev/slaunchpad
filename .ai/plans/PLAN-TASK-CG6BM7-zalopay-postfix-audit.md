...
## Appendix 5 — Round 5 steps (35+)

35. `RefundInterface`: const `PROVIDER_REQUEST_STARTED_AT` + state `provider_request_started` (state machine v5, DEC-007). 36. `db_schema.xml` + whitelist: cột `provider_request_started_at` datetime nullable. 37. `PendingRefundManager`: `RECONCILIATION_GRACE_SECONDS = 120`, `markProviderRequestStarted()` UPDATE claim-guarded pin state+timestamp, hasInFlight/saveState 5 state (F23). 38. Plugin: gate `markProviderRequestStarted` giữa bind và `executePrepared`; fail ⇒ terminate abandoned + "could not be started" (F23). 39. Cron: step 0b MỌI initiating → terminate abandoned (LOCAL_READY, không query); step 0c grace (missing-ts ⇒ consumeQueryBudget; trong grace ⇒ no-op; hết grace ⇒ identity query) (F23). 40. `BackfillRefundState` cohort 3: quoteInto một-placeholder-một-lời-gọi (F24). 41. Tests: cron F23 ×4 (fresh-grace/stale/missing-ts/bound-initiating rewrite), manager ×2, plugin ×1 + REQUIRED order `['save','bind','start','provider']`, backfill pin 2 quoteInto + exact WHERE → **312/1126 OK**. 42. PHPCS 0 errors; php -l từng file. 43. REAL DB no-defer (F25): setup:install PASS (MariaDB 10.6) → SHOW CREATE TABLE đủ 4 điều kiện + cột mới → drop-cột + xóa patch entry → setup:upgrade PASS (delta + patch re-run) → seed legacy A/B/C/D + multi-row → cohort verify + F24 WHERE thật → claim proof `acquireClaim` thật (unresolved REJECTED / resolved ALLOWED / 1 owner MIN) → setup:di:compile PASS. 44. CodeGraph anchors + audit độc lập; commit + push chỉ `task/zalopay-postfix-audit`; receipt round 5; STOP cho TL.

## Appendix 6 — Round 6 steps (45+)

45. `PendingRefundManager`: const `LOCAL_READY_GRACE_SECONDS = 300` + docblock rationale (pha cục bộ không network, headroom >300×; RIÊNG BIỆT với `RECONCILIATION_GRACE_SECONDS = 120`). 46. `RefundInterface`: const `CREATED_AT = "created_at"` (anchor bền — set lúc INSERT claim, không bao giờ đổi; không đụng schema). 47. Cron step 0b viết lại (F26, DEC-008): missing `created_at` ⇒ `consumeQueryBudget(reconcile)` bounded + không nhả; fresh (< 300s) ⇒ no-op hoàn toàn (info log + return); stale (≥ 300s) ⇒ terminate `abandoned_before_provider_io: stale LOCAL_READY claim - provider I/O impossible by construction` → `confirmed_fail` + nhả claim. 48. Tests cron: −2 round-5 initiating +6 F26 (fresh unbound/bound NEVER mọi mutation + row pin; stale unbound/bound terminate EXACT evidence + `confirmed_fail` + repo/command NEVER; missing-ts consume-only; race composition `testRaceCronDuringLocalBindLeavesOwnerFullContinuation`) → **316/1145 OK**. 49. Real-DB race proof P20 (cron thật qua DI, scopeConfig override; MariaDB 10.6 disposable): R1 fresh unbound untouched + mark true; R2 stale unbound crash recovery confirmed_fail + evidence abandoned (provider count 0) + row F23 vẫn untouched cùng run; R3 bound FK-thật fresh untouched + stale nhả giữ `credit_memo_id`; side-proof 1062 atomic claim. 50. Validation: php -l; PHPCS 0 errors; setup:di:compile PASS (/tmp/m2b re-stage); CodeGraph re-index + anchors; SCHEMA_CHANGED=NO ⇒ không setup:upgrade lại (P19 còn hiệu lực). 51. Docs: findings F26, proofs P20, validation round 6, DEC-008, SPEC Rev 7, TASK round 6; commit `[Zalo]` + push CHỈ `task/zalopay-postfix-audit`; receipt round 6; STOP cho TL.

## Appendix 7 — Round 7 steps (52+, DEADLINE MODE)

52. Marker v2 (F28, DEC-009): `RefundOutcomeMarker` API `authorize(int)/consume(int): bool/clear(int)` keyed `credit_memo_id`, one-shot, try/finally; `RefundCommand::prepare()` consume-null ⇒ plugin fail-closed (logger->error, KHÔNG `$proceed()`). 53. Plugin lifecycle (F27/F31/F34, DEC-011): `setState(STATE_OPEN)` + invoice guard + `setInvoiceId()` trước pre-provider save; xóa `Plugin/Model/Order/CreditmemoPlugin.php` + `zalopay_creditmemo_states` (getStates() static — interception chết). 54. Phân loại typed (F30, DEC-014): `Exception/RefundProtocolException`; RefundCommand rc-bảng 1/2/3/missing/nonnumeric/unexpected; rc=3 + 1 query tức thời; plugin catch Transport → Protocol (`markUnknown`) → Localized (`markConfirmedFail`), rethrow nguyên gốc. 55. Manager CAS (F29, DEC-010): mọi transition conditional UPDATE (`refund_state` FROM-guard + `active_claim=1`), `consumeQueryBudget` atomic increment KHÔNG đổi state (F32), `finalizeSuccess` FOR UPDATE + CAS + marker try/finally quanh `RefundOperation`. 56. Cron (F32/F33, DEC-015): selection terminal-explicit `is_processed=0 AND state IN (5 unresolved) AND (attempts<96 OR PSLP)`; guard đầu `processRefund`; xóa `releaseCreditmemoAfterFail`. 57. Schema (F35/F36): FK NO ACTION; `RefundCleanupCronjob` retention 90d chỉ confirmed_success. 58. Blocker real-gate: `Rate::getVndAmount` untyped + `resolveOrderCurrencyCode()` (OrderAdapter `getCurrencyCode()` vs Order `getOrderCurrencyCode()`). 59. Tests: marker/plugin/command/cron/manager/cleanup rewrite + mới → **364/1299 OK**; PHPCS 0 errors; lint 121 files. 60. REAL gate (P21): sync SUCCESS, UNKNOWN→cron→SUCCESS cùng m_refund_id, explicit FAIL, marker isolation same-process, CAS stale-writer 2-process — 5/5 PASS; setup:upgrade + SHOW CREATE TABLE FK NO ACTION. 61. Docs: DEC-009…015, findings F27–F36, P21, validation round 7, SPEC Rev 8, TASK round 7; commit + push CHỈ `task/zalopay-postfix-audit`; receipt deadline; STOP cho TL.
