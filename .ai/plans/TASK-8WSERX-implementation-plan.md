# Kế hoạch triển khai: TASK-8WSERX — Apply VN 2-level address dropdown trên admin Store Information + Shipping Origin config

| Field | Value |
|---|---|
| Specification | specs/SPEC-TASK-8WSERX-admin-vn-address-store-config.md |
...
> Parent: [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md). Ticket: [TASK-8WSERX](../tickets/TASK-8WSERX-apply-address-dropdown-admin-store-information.md). Spec: [admin-vn-address-store-config](../specs/SPEC-TASK-8WSERX-admin-vn-address-store-config.md).

## Metadata

| Field | Value |
|-------|-------|
| Ticket | [TASK-8WSERX](../tickets/TASK-8WSERX-apply-address-dropdown-admin-store-information.md) |
| Spec | [admin-vn-address-store-config](../specs/SPEC-TASK-8WSERX-admin-vn-address-store-config.md) |
| Feature | [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md) |
| Author | AI draft |
| Reviewer (TL) | user — **pending plan approval** (Level-2) |
| Workflow Mode | B |
| Date | 2026-08-07 |
| Decisions | DEC-FEATE2HM1J-001 (accepted) · DEC-TASKYJENM2-001 (accepted) · DEC-FEATJSZQV3-003 · DEC-17 |
| Validation level | **L3** (config save → downstream PDF/print origin + carrier/TableRate origin — verify system-wide) |

## 1. Hướng tiếp cận

**Tách generic mechanism (`Secomm_AddressDropdown`) vs country adapter (`Secomm_VietNamAddress`) — DEC-FEATJSZQV3-003/025.** Inject dependent ward dropdown vào field `city` của core config qua `frontend_model` (Magento-idiomatic), reuse generic data + cascade JS precedent của TASK-SQY42T (`address-city.js` suffix-selector — vốn đã tương thích config form), và validate VN trên config save mirroring TASK-4ZV5NG precedent.
...
| `Secomm/AddressDropdown/view/adminhtml/requirejs-config.js` | verify/edit | đăng ký path/mixin nếu factor JS ra lib riêng (đảm bảo không vỡ order binding TASK-SQY42T) |
| `Secomm/AddressDropdown/Model/Resolver/GetListCityGraphql.php` (+ `GetListSubCity`) + `CityLocaleCollection` | **KHÔNG sửa** (reuse read-only) | data layer generic — DEC-FEATJSZQV3-003 |
...
| **NEW** `Secomm/VietNamAddress/Observer/Adminhtml/Config/ValidateVietNamWard.php` (hoặc `Plugin/Adminhtml/Config/ValidateVietNamWard.php`) | new | gate `country==VN` + path ∈ {`general/store_information`, `shipping/origin`}; ward ∈ province qua `CityLocaleCollection` filter `(region_id, default_name)`; invalid → neutralise `city` + log warning masked; catch `\Exception` non-fatal (mirror TASK-4ZV5NG plugin) (AC-3/validate) |
| `Secomm/VietNamAddress/etc/module.xml` | verify | sequence load sau `Secomm_AddressDropdown` (để VN validate/label apply sau generic — đã có từ TASK-4ZV5NG, verify) |
| `Secomm/VietNamAddress/i18n/vi_VN.csv` + `en_US.csv` | verify/edit | "Phường/Xã"/"Ward/Commune" (đã có từ TASK-4ZV5NG — verify; add nếu thiếu cho config context) |
| **KHÔNG sửa** storefront `view/frontend/*`, customer/order admin surface (TASK-4ZV5NG/012), MSI (TASK-2V0AEV), `db_schema.xml`, GraphQL resolvers, carrier/TableRate algorithm | — | DEC-FEATJSZQV3-003; reuse read-only; downstream verify-only (AC-5/6/7) |
...
2. **Generic config frontend_model + phtml (AddressDropdown)** — risk: medium — deps: 1
   ...
   - `City.php` block: `_getElementHtml()` render native `<input>` (value carrier — giữ `$element->getName()`/`$element->getHtmlId()` + value đã lưu) + `<select>` `city_select` + `<select>` `sub_city` + `data-mage-init` JSON (`cityInputId`, `citySelectId`, `subCitySelectId`, `currentCity`, `currentSubCity`). Mirror `Block/Adminhtml/Order/Address/Renderer/City.php` (TASK-SQY42T).
   - `city.phtml`: markup 3 wrap (`.secomm-config-city-input/.select/.sub-city`) — mirror order `city.phtml` (TASK-SQY42T).
...
4. **Server-side validate (VietNamAddress) — config save** — risk: medium — deps: none (parallel-safe với 2/3)
   - Observer `model_config_data_save_before` (hoặc plugin `Magento\Config\Model\Config::save`): scope section/group ∈ {`general/store_information`, `shipping/origin`} + `country==VN`; ward ∈ province qua `CityLocaleCollectionFactory` filter `(region_id, default_name)`; invalid → unset `city` trong payload + log warning masked; catch `\Exception` → log error, không rethrow (non-fatal). Reuse logic `ValidateVietNamWard::beforeSave` (TASK-4ZV5NG).
...
5. **i18n + module sequence** — risk: low — deps: 3
   - Verify "Phường/Xã"/"Ward/Commune" trong `vi_VN.csv`/`en_US.csv` VietNamAddress (TASK-4ZV5NG đã add); verify `module.xml` sequence `VietNamAddress` after `AddressDropdown`.
...
7. **Tests + QC + evidence (L3)** — risk: low — deps: 2,3,4,5,6
   ...
   - evidence → `.ai/runtime/evidence/FEAT-E2HM1J/` (TASK-8WSERX).
...
| Region updater mặc định xung đột cascade JS → region/city lệch | medium | reuse hydration watcher `(country\|region_id)` của `address-city.js` (proven TASK-SQY42T cho admin region async); QC country switch + region change + reload |
| Factor `address-city.js` (order ↔ config share) → double-bind/vỡ order TASK-SQY42T | high | ưu tiên factor cẩn thận + verify order binding TASK-SQY42T nguyên vẹn; nếu rủi ro cao → thin config variant riêng (không share state) |
...
- **Regression**: config field khác trong 2 form + section khác; config save/cache clean; PDF hiện hành; TableRate rate hiện hành; storefront; admin surface khác (customer TASK-4ZV5NG, order TASK-SQY42T) nguyên vẹn.
- **High-risk**: config value carrier field name (persist); PDF downstream (gửi khách); generic JS factor (order TASK-SQY42T không vỡ).

## 6. Out of scope

- Các admin surface khác: customer address (TASK-4ZV5NG), order create/edit (TASK-SQY42T), MSI Source (TASK-2V0AEV).
- Storefront generic/VN cleanup (TASK-KCBDDT) — coordinate, không overlap.
- ward_id / `city_id` canonical persistence (path A, DEC-TASKYJENM2-001 long-term) — ward persist name string.
...
- Coordinate với **TASK-SQY42T** (generic `address-city.js` factor) — nếu TASK-SQY42T đang in-progress, chốt factor point chung để tránh conflict.

## 8. Status (post-implementation, pre-TL-review — 2026-08-07)

Plan approved (user as TL); implemented Steps 1–7. Evidence: [`.ai/runtime/evidence/FEAT-E2HM1J/TASK-8WSERX.md`](../runtime/evidence/FEAT-E2HM1J/TASK-8WSERX.md). **Pending QC L3 + TL code review.**
...
**Step 3 — dedicated config JS instead of factoring TASK-SQY42T's `address-city.js` (justified).**
The plan offered two options: factor the generic order `address-city.js` (TASK-SQY42T) into a shared
lib, or a self-contained config variant (fallback). Audit found TASK-SQY42T's order mechanism files
(`Renderer/City.php`, order `city.phtml`, `web/js/order/address-city.js`) are **not present** in
the working tree — TASK-SQY42T is in-progress and its JS state is volatile. To avoid coupling TASK-8WSERX to
...
(`escapeGraphQlValue`). No shared state with TASK-SQY42T → no regression risk to TASK-SQY42T's binding.
