# SL-020 Implementation Plan — Scaffold Secomm_Promotion + Secomm_PromotionMaxDiscount

| Field | Value |
|---|---|
| Specification | tickets/SL-020-scaffold-promotion-modules.md (`## Mini Spec`, embedded — ID SL-020) · canonical parent specs/SPEC-FEAT-008-promotion-max-discount.md (FULL, VALID) §3 |

> **Mode C** · Tier 1 · Status: **Retro-canonical** — distilled 2026-08-19 từ work đã Dev complete (evidence: [SL-020-evidence](../runtime/evidence/SL-020/SL-020-evidence.md)); mọi task đánh dấu theo trạng thái thực.
> Boundary nguồn: DEC-FEAT008-001 §5 (module boundary) + FEAT-008 spec §3.
...
|---|---|---|
| Base module chứa gì? | **Đúng 5 files, 0 PHP class** — anchor thuần; contracts chỉ rút lên khi có promotion module thứ 2 (YAGNI) | DEC-FEAT008-001 §5 |
...
| Format header/style? | Theo precedent mới nhất `Secomm_ShippingCore` (SL-015): header `@author Secomm Team`, module.xml có comment tham chiếu DEC | `app/code/Secomm/ShippingCore/` |
| Sequence của feature module? | `Secomm_Promotion` + `Magento_SalesRule` + `Magento_Quote` + `Magento_Sales` (Magento_Sales cho downstream order/invoice/CM của SL-024) | DEC-FEAT008-001 §5, spec §3 |
...
Task 1–3 (scaffold)  →  Task 4 (static)  →  Task 5 (runtime; bị block MySQL ~nửa ngày, resume xanh)
                                        ...
                                        merge cùng nhánh feature FEAT-008 (không deploy riêng)
```
...
2. **SL-021** (data model — Mode B, Tier-2 DB) kế tiếp pipeline; SL-023/024 chạy song song sau SL-022.
3. Release checklist gộp ở cấp FEAT-008 (SL-025), không riêng SL-020.
