...
## P22 — Deadline micro-correction: terminate() state-CAS (real DB + unit)

- **Unit**: `testStaleInitiatingCannotKillProviderStarted` (snapshot initiating/claim=1, DB provider_request_started ⇒ 1 CAS attempt duy nhất, WHERE pin `refund_state='initiating'`, không overwrite, claim giữ), `testStaleProcessingCannotOverwriteNewerUnknownState`, `testNormalTerminatePersistsWhenSnapshotMatchesDb` (affected=1, terminal persist); WHERE pins cập nhật trong 2 test CAS cũ. Suite **367 tests / 1316 assertions OK**.
- **Real-DB focused proof (env có sẵn /tmp/m2r + MariaDB 10.6, KHÔNG build gì mới)**: INSERT row thật `initiating/claim=1` (order 13) → `cas_race.php load 14` freeze snapshot → owner THẬT `markProviderRequestStarted` ⇒ DB `provider_request_started`/claim=1 → stale cron `fire-terminate` (intent confirmed_fail) ⇒ **affected=0: row vẫn `provider_request_started`, claim=1, last_error NULL** (STALE_INITIATING_CANNOT_KILL_PROVIDER_STARTED=PASS). Sau đó snapshot processing == DB processing ⇒ terminate affected=1 ⇒ `confirmed_fail` + claim NULL + evidence đúng (NORMAL_TERMINATE=PASS). Unit pins Test 2 (processing vs unknown).
