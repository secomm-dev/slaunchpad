...
spec_status: VALID           # approved 2026-08-19 (user acting as TL) — DEC-FEAT008-001 accepted
specification_ref: ../../specs/SPEC-FEAT-008-promotion-max-discount.md
risk: high
status: proposed
created: 2026-08-19
updated: 2026-08-19
ticket_ref:
  - SL-020                   # scaffold Secomm_Promotion + PromotionMaxDiscount (Mode C)
  - SL-021                   # db_schema column + extension attribute mapping (Mode B, Tier-2 DB)
  - SL-022                   # cap engine: collector 310 + LRM + unit tests (Mode A core)
  - SL-023                   # admin UI: ValueProvider plugin + JS field + i18n (Mode B)
  - SL-024                   # integration tests + idempotency + lifecycle (Mode A)
  - SL-025                   # QC matrix + OSC e2e + context docs (Mode B)
decisions:
  - DEC-FEAT008-001          # hook collector 310, data model, LRM redistribution, module boundary (accepted)
decision_assessment: material
decision_refs: [DEC-FEAT008-001]
decision_approval_summary:
  total: 1
  pending_approval: []
  approved: [DEC-FEAT008-001]
  rejected: []
  superseded: []
...
# Feature Record: Promotion Max Discount (Maximum Discount Cap)

<!-- CANONICAL RECORD — Full analysis/spec: SPEC-FEAT-008. Backend-first, theme-agnostic,
...
- **[SL-020](../../tickets/SL-020-scaffold-promotion-modules.md)** — scaffold `Secomm_Promotion` (base tối giản) + `Secomm_PromotionMaxDiscount` (skeleton). → `proposed` (Mode C).
- **[SL-021](../../tickets/SL-021-maxdiscount-db-schema.md)** — db_schema column `maximum_discount_amount` + whitelist + extension attribute cho `RuleInterface` + RuleRepository mapping. → `proposed` (Mode B, **Tier-2 DB schema**).
- **[SL-022](../../tickets/SL-022-maxdiscount-cap-engine.md)** — cap engine: collector 310 + CapResolver + LRM allocator + unit tests. → `proposed` (Mode A, Tier-2 pricing — core).
- **[SL-023](../../tickets/SL-023-maxdiscount-admin-ui.md)** — admin UI: ValueProvider plugin + JS switch field + i18n vi/en. → `proposed` (Mode B).
- **[SL-024](../../tickets/SL-024-maxdiscount-integration-tests.md)** — integration tests (quote matrix A–D + lifecycle E) + idempotency. → `proposed` (Mode A, Tier-2 order lifecycle).
- **[SL-025](../../tickets/SL-025-maxdiscount-qc-docs.md)** — QC matrix + OSC e2e + project-context docs. → `proposed` (Mode B).

Thứ tự thực thi: SL-020 → SL-021 → SL-022 → (SL-023 ∥ SL-024) → SL-025.
...
- → [DEC-FEAT008-001](../decisions/DEC-FEAT008-001.md) (accepted).
...
- Spec: [SPEC-FEAT-008](../../specs/SPEC-FEAT-008-promotion-max-discount.md) (18-mục analysis: native behavior, hook-point, data model, redistribution, rounding, tax, shipping, idempotency, lifecycle, edge cases, compat, AC, test strategy, plan, estimate)
