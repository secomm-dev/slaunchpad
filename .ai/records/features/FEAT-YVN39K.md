...
parent: null
legacy_ids: [FEAT-001]
title: Dual-theme (Luma + Hyva) AddressDropdown — MODULE layer (TASK-88NDV5)
...
created: 2026-07-16
updated: 2026-07-21
ticket_ref: TASK-88NDV5           # legacy ticket — consolidated here (read-only)
...
supersedes: []               # consolidates TASK-88NDV5 legacy artifacts (ticket/spec/plan/testcase)
---

# [SLP][FEAT-YVN39K] Dual-theme (Luma + Hyva) AddressDropdown — MODULE layer (TASK-88NDV5)

<!-- CANONICAL RECORD (Phase 1a) — consolidates legacy TASK-88NDV5 ticket + spec + plan + testcase. -->
<!-- Legacy sources below are read-only / superseded; this record is the single source of truth for TASK-88NDV5. -->
<!-- Raw evidence → .ai/runtime/evidence/FEAT-YVN39K/ (Phase 1a permissive; runtime carve-out is Phase 1d). -->

## Context

Migrate the **generic frontend surfaces** of `Secomm_AddressDropdown` (customer address form + cart shipping estimation) to be **dual-theme** (Luma native jQuery/Knockout + Hyva native Alpine/Magewire). MODULE layer only — reusable, theme-agnostic, **no OSC coupling** (OSC integration → TASK-FMAN1B / Launchpad). Backend (`Api/`, `Model`, `Setup`, import, admin UI) untouched. Satisfies BR-002 (module portion) + BR-001 (i18n vi/en) on both themes.

**Scope boundary (DEC-8):** module owns generic Magento-native surfaces; OSC coupling forbidden in module → belongs to Launchpad (TASK-FMAN1B). Dead `Secomm_Ahamave` ref removed (AC-010).

**Risk:** Tier 2 (`Secomm_AddressDropdown` GraphQL surface — §12) → SA/TL escalation.

Legacy sources consolidated (read-only): [tickets/TASK-88NDV5](../../tickets/TASK-88NDV5-apply-hyva-theme-addressdropdown.md) · [specs/addressdropdown-hyva](../../specs/addressdropdown-hyva.md) · [plans/TASK-88NDV5](../../plans/TASK-88NDV5-implementation-plan.md) · [testcases TASK-88NDV5 portion](../../testcases/TASK-88NDV5-TASK-FMAN1B-testcases.md).
...
**DEC-8 — Reusability boundary**: module owns generic Magento-native surfaces; OSC coupling forbidden in module → Launchpad (TASK-FMAN1B).
...
**TC-001..TC-016** (from `testcases/TASK-88NDV5-TASK-FMAN1B-testcases.md`, TASK-88NDV5 portion). Common preconditions: theme active, VN 3-tier data via `InstallVietNamAddressPatch`/`VN_Address_2Level.csv`, locales vi_VN + en_US.
...
> Raw test output → `.ai/runtime/evidence/FEAT-YVN39K/` (runtime; Phase 1d carve-out pending).
...
- **AC-011 / Q1 deferred:** resolver `GetListCity`/`GetListSubCity` are `@deprecated` + `@cache(false)` → potential N+1 on cascade. Hardening (undeprecate + cache) is Tier 2 SA work, deferred out of TASK-88NDV5.
...
- Legacy sources (read-only): [tickets/TASK-88NDV5](../../tickets/TASK-88NDV5-apply-hyva-theme-addressdropdown.md) · [specs/addressdropdown-hyva](../../specs/addressdropdown-hyva.md) · [plans/TASK-88NDV5](../../plans/TASK-88NDV5-implementation-plan.md) · [testcases (TASK-88NDV5 portion)](../../testcases/TASK-88NDV5-TASK-FMAN1B-testcases.md)
...
- Related record: TASK-FMAN1B (Launchpad OSC integration) — pending consolidation (spec `_TBD_`)
