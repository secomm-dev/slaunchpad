...
decision_refs: [DEC-FEATHEHJQ4-001, DEC-FEATHEHJQ4-002, DEC-FEATHEHJQ4-003, DEC-FEATHEHJQ4-004, DEC-FEATHEHJQ4-005, DEC-FEATHEHJQ4-006]   # B1 explicit alias of decisions:
decision_approval_summary:   # DERIVED (B1) — single-read approval state; all 6 DEC are `proposed`
  total: 6
  pending_approval: [DEC-FEATHEHJQ4-001, DEC-FEATHEHJQ4-002, DEC-FEATHEHJQ4-003, DEC-FEATHEHJQ4-004, DEC-FEATHEHJQ4-005, DEC-FEATHEHJQ4-006]
  approved: []
  rejected: []
  superseded: []
...
# [SLP][FEAT-HEHJQ4] VNPAY IPN callback processing — signature, status mapping, retry, idempotency, audit
...
| **decision_assessment** | **required → done (`material`)** | 13-dimension assessment; material choices ⇒ DEC-FEATHEHJQ4-001..DEC-FEATHEHJQ4-006 (canonical, `proposed`) |
...
## Open decisions (SA/TL — captured as canonical DEC-FEATHEHJQ4-001..DEC-FEATHEHJQ4-006, status `proposed`)
...
1. **Signature (AC-001, DEC-FEATHEHJQ4-001):** extract a `SignatureVerifier` (`hash_equals` + confirmed VNPAY HMAC-SHA512 construction); reject → `RspCode '97'` + audit; never disclose which field failed.
2. **Idempotency + lock (AC-004/005, DEC-FEATHEHJQ4-004):** wrap processing in a DB transaction + order/tx lock keyed on `vnp_TransactionNo`; register payment via `registerCaptureNotification` (no manual `setTotalPaid`); invoice+payment+order in one transaction.
3. **Status mapping (AC-002, DEC-FEATHEHJQ4-002):** apply the SA-ratified `vnp_ResponseCode`/`vnp_TransactionStatus` → state table (07 suspect → PAYMENT_REVIEW/HOLD, not auto-process/cancel).
4. **Retry (AC-003, DEC-FEATHEHJQ4-003):** early-callback path — respond retry-eligible OR enqueue with bounded retries + backoff; distinguish "order not found" (terminal) vs "not yet available" (retry).
5. **Audit (AC-006, DEC-FEATHEHJQ4-005):** write immutable audit on every terminal branch (rejected / retried / duplicate / success).
6. **Payload compat (AC-007, DEC-FEATHEHJQ4-006):** defensive parse + version detect + fail-safe on unknown mandatory fields.
...
- **Payload compatibility (AC-007, DEC-FEATHEHJQ4-006):** current VNPAY payload parsed; unknown optional fields tolerated; unknown mandatory fields → fail-safe reject + audit (no silent corruption).
...
- Decisions: **DEC-FEATHEHJQ4-001..DEC-FEATHEHJQ4-006** (canonical, status `proposed`) — see §Open decisions + [`.ai/records/decisions/`](../decisions/). Each DEC backlinks to FEAT-HEHJQ4 (Related records).
