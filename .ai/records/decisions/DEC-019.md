...
<!-- Concretizes DEC-8 (boundary) for i18n labels + extends DEC-018 (per-carrier code mapping). -->
...
3. **Per-carrier code mapping → Launchpad (DEC-018).** Mã province/ward riêng của từng carrier (GHN, Ahamove, TableRate) nằm trong **customization từng carrier ở Launchpad**, KHÔNG thuộc generic hay country module.
   - **Cart estimator** (FEAT-005/SL-003) chỉ thu thập **định danh admin VN** (province = Magento region, ward = default_name → native `city`). **KHÔNG mang carrier code.**
...
- **Carrier code mapping trong country module:** rejected — country module sẽ coupling carrier 3rd-party → mất reusable (DEC-8/DEC-018).
...
- [FEAT-005](../features/FEAT-005.md) — VN shipping address support
- [DEC-017](DEC-017.md) — generic VN address capability → VietNamAddress
- [DEC-018](DEC-018.md) — per-carrier customization → Launchpad
- DEC-8 (generic vs market-specific boundary) — concretized here for labels
- DECISIONS.md index: DEC-019
