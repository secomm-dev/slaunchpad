...
supersedes: []
superseded_by:
work_items: [SL-018, SL-020]   # primary = SL-018 (subject-owner: spec-first governance stream); trigger = SL-020
---

# Decision Record: Spec-First ticket activation hardening (extends DEC-SL018-001)
...
<!-- CANONICAL DECISION STORE. ACCEPTED 2026-08-19 — directed by user acting as TL (chat:
     ...
     DEC-SL018-002 theo precedent DEC-SL018-001/DEC-SL019-001 (user correction 2026-08-19 —
     process decisions có governance work-item anchor theo format per-work-item, KHÔNG global DEC-NNN).
     -->
<!-- Đổi tên từ DEC-026 (global) → DEC-SL018-002 theo chỉ đạo user 2026-08-19: precedent
     DEC-SL018-001/DEC-SL019-001 cho process decisions có work-item anchor. -->
<!-- Index pointer: `.ai/project-context/memory/DECISIONS.md` -->

## Context (audit — vì sao gate không chặn SL-020)

Slaunchpad 2026-08-19: user yêu cầu "implement ticket SL-020". Ticket khi đó **chưa có spec
và plan riêng** (chỉ có dòng `**Specification:**` reference parent Full-Spec SPEC-FEAT-008 +
...
- **G1 — Reference-satisficing escape:** rule cho "Full Spec reference" **HOẶC** "Mini-Spec
  ...
  với tinh thần DEC-SL018-001 ("Ticket+AC never substitutes for specification"): AC-only thì
...
Ghi nhận thêm: SL-008/SL-009 ở status "Ready" stale từ 2026-07-30 dù đã shipped trong
FEAT-006 — legacy debt bị surfacing khi hardening (đã reconcile).
...
4. **Migration:** không bulk-rewrite historic tickets (theo migration strategy sẵn có của
   ...
   reconcile 2 ticket stale (SL-008/009 "Ready" → "Dev complete" kèm byline, đã shipped trong
   FEAT-006). SL-021..SL-025 đang Proposed — sẽ cần Mini-Spec + Plan/Approach khi activate.
...
- **WARN-only rollout như migration strategy gốc** — case SL-020 chứng minh satisficing
...
- Trigger audit: [SL-020](../../tickets/SL-020-scaffold-promotion-modules.md) (+ Mini-Spec/plan bổ sung post-hoc 2026-08-19)
- Extends: [DEC-SL018-001](DEC-SL018-001.md) (giữ nguyên, không supersede — hardening thêm tầng ticket)
