...
<!-- CANONICAL DECISION STORE. ACCEPTED 2026-09-08 — approved by user acting as SA/TL in chat (precedent DEC-TASKZ132WA-001): approach quote-based intent + scope gọn trong Vnpayment_VNPAY (không module mới) + loại Secomm_PaymentCore (FEAT-CSWYEJ) khỏi phương án. TL code review vẫn bắt buộc trước merge (§8.3/§11). -->
...
Đổi `Vnpayment_VNPAY` sang **quote-based payment intent — order chỉ được tạo sau khi VNPAY xác nhận thành công**:
...
6. Scope: mọi thay đổi nằm gọn trong `Vnpayment_VNPAY` — không tạo module mới, không schema migration. `Secomm_PaymentCore` (FEAT-CSWYEJ) không còn sử dụng — không tham chiếu trong approach.
