...
<!-- Supersedes the earlier "(region_id, ward_name) key + fallback đảm bảo API luôn hoạt động" assumption in FEAT-006 / SL-008. -->
...
<!-- CORRECTION 2026-07-31 (user): VN 2-level model → "ward" (phường/xã) = the CITY level, so ward_id = directory_region_city.city_id (NOT directory_city_sub_city.sub_city_id, which is the deprecated 3rd level). FEAT-005 U3 confirms: GetListCity(region_id) sources the Ward. ward name (vi_VN) = directory_region_city_name.name. -->
...
FEAT-006 (GHTK carrier) ban đầu định nghĩa mapping key `(region_id, ward_name)` và mô tả fallback vi_VN là "đảm bảo API luôn hoạt động". Hai vấn đề sau được phát hiện khi chuẩn bị implementation:
...
- **VN 2-level model**: province = `directory_country_region`; ward (phường/xã) = **city level** (`directory_region_city`, PK `city_id`, có `region_id`). Sub-city (`directory_city_sub_city`) là mức 3 cũ → **KHÔNG dùng cho VN 2-level** (FEAT-005 U3: `GetListCity` cho Ward). Vậy stable ward identifier = **`directory_region_city.city_id`**.
...
5. **ward_id availability gap — RESOLVED (path B, accepted 2026-07-30):** chọn **path B — GHTK-internal name→`city_id` bridge** (`WardIdBridge` trong `Secomm_Ghtk` resolve `city_id` từ `directory_region_city` theo `(region_id, default_name)` tại lookup; không đổi generic module → tôn trọng DEC-019/DEC-8). Path B là compatibility/technical-debt tạm; **path A (expose + persist `city_id` ở address data layer)** ghi là long-term follow-up (ticket riêng khi decide). Bridge phải deterministic (name trong region đủ định danh).
...
- (+) Path B không chạm generic module → SL-008 unblocked (→ ready).
- (−) Path B = tech-debt tạm (bridge name→`city_id`); path A (long-term) sinh ticket generic module (DEC-019 boundary).
...
- Sub-tickets: [SL-008](../../tickets/SL-008-ghtk-address-mapping.md).

## Related records

- Features: [FEAT-006](../features/FEAT-006.md) · [FEAT-005](../features/FEAT-005.md)
- Decisions: [DEC-018](DEC-018.md) · [DEC-019](DEC-019.md) (generic vs carrier ownership) · DEC-8 (boundary)
- DECISIONS.md index: DEC-020
