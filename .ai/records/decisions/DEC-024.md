...
<!-- These are NOT yet decided — recorded as pending so SL-010 is not silently closed. Do NOT treat as accepted. -->
<!-- 2026-07-30 partial resolution: pick_money/is_freeship (B2/B3) → DEFERRED; cancel API (B9) → OUT OF PHASE (open). Rest (B1/B4/B6/B7/B8/B10–B14) still pending → SL-010 stays `proposed`. -->
<!-- 2026-07-30 PARKED: order sync (SL-010) deferred (fee-first) → DEC-024 not blocking fee delivery. -->
...
SL-010 (order sync) chưa đủ ready vì nhiều business rule chưa khóa. GHTK order API (`POST /services/shipment/order`) cần các giá trị cụ thể mà mapping từ Magento chưa xác định. Ghi lại thành một bundle decision với mỗi điểm ở trạng thái **pending approval** — không tự suy đoán rồi ghi như đã approved.
...
| B11 | Manual retry | admin action retry (tái submit với cùng partner_order_id) — reuse DEC-023 outbox | SA/TL | pending |
...
- **Tự chọn ngầm mỗi rule để "đóng" SL-010** — rejected: sai (user yêu cầu tạo pending decision, không tự chốt); rủi ro tài chính/operational nếu đoán sai COD/declared value.
...
- (−) **SL-010 PARKED (fee-first)**; khi resume: B1/B2 (COD/pick_money) **blocked trên COD BA outcome** (order status + tiền mặt vs chuyển khoản); B4 (declared value) / B8 (partner order ID) / B14 (sync trigger) pending approval; B3/B9 defer/out-of-phase.
...
- Decisions: [DEC-023](DEC-023.md) (async/idempotency architecture this bundle rides on) · [DEC-021](DEC-021.md) · [DEC-022](DEC-022.md)
- DECISIONS.md index: DEC-024
