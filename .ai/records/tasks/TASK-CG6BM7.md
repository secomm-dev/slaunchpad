...
## Round 7 (2026-09-17, DEADLINE MODE — TL source review lần 7, PRE_HEAD `c8c59231`)

- Chế độ DEADLINE MODE: tối đa 2 worker lane, không infra mới, minimum release gate; DEFER ghi nhận minh bạch.
- P0 fix: **F27** (CM persist pre-provider = native `STATE_OPEN`, xóa custom state 4 + plugin static-interception chết — F34; DEC-011); **F28** (one-shot marker `authorize/consume/clear` keyed `credit_memo_id`, `prepare()===null` ⇒ fail-closed KHÔNG gọi `$proceed()` — DEC-009); **F30** (chỉ `return_code=2` = CONFIRMED_FAIL; missing/non-numeric/unexpected ⇒ `RefundProtocolException` → UNKNOWN, giữ claim + m_refund_id — DEC-014); **F31** (persist `invoice_id` pre-provider; confirmed-success khôi phục association ⇒ ONLINE refund — DEC-011); **F32** (PSLP không bao giờ thành UNKNOWN: `consumeQueryBudget` atomic increment KHÔNG đổi state; cron selection PSLP-at-cap — DEC-015).
- P1 fix: **F29** (CAS conditional UPDATE cho mọi transition sau claim; CAS-fail ⇒ reload + log, không overwrite — DEC-010); **F33** (terminal confirmed_success/fail không bao giờ được cron chọn lại — guard 2 tầng SQL + processRefund).
- Done-sống (correct + green, giữ theo rule deadline): **F35** (FK CASCADE → NO ACTION — schema đổi thật, setup:upgrade + SHOW CREATE TABLE xác minh), **F36** (retention evidence 90 ngày — cleanup cron mới + tests). Không phải deferred.
- Phát hiện trong REAL gate (blocker money-path, đã fix): `Rate::getVndAmount` type-hint `Order` trong khi gateway truyền `OrderAdapter` ⇒ TypeError crash mọi synchronous refund trước khi provider được hỏi. Fix duck-typed qua `resolveOrderCurrencyCode()` (Order = `getOrderCurrencyCode()`, OrderAdapter = `getCurrencyCode()`).
- Minimum release gate — TẤT CẢ PASS trên REAL Magento 2.4.8-p5 (disposable env /tmp/m2r, MariaDB 10.6, currency VND, fake provider transport — accountingMagento REAL): unit **364 tests / 1299 assertions OK**; php -l 121 files clean; PHPCS Magento2 **0 errors** (module + changed scope); setup:di:compile PASS; setup:upgrade PASS + FK `ON DELETE NO ACTION` xác minh SHOW CREATE TABLE; REAL smokes: sync SUCCESS (CM REFUNDED, invoice_id, invoice/order/payment refunded totals, online refund, refund txn, /refund=1), async recovery UNKNOWN→cron query CÙNG m_refund_id→confirmed_success (/refund=1, query=1), explicit FAIL (CM OPEN, confirmed_fail, claim nhả, không accounting), marker isolation 2 refund cùng process (/refund=2, 2 row độc lập), CAS stale-writer 2 process (stale fire-terminate affected=0, row giữ confirmed_success).
- .ai: DEC-009…015; findings F27–F36; proofs P21; validation round 7; SPEC Rev 8; PLAN Appendix 7.
- DEFERRED (POST-DEADLINE HARDENING): broad cleanup refactors, exhaustive DB concurrency matrix, admin UX, Debug Mode config, logo/icon, New Order Status, email corner cases, generic test harness, PHPCS warnings (style, 0 errors), các file pre-existing warnings (RefundInterface/PaymentAttemptInterface/ReturnProcessor).
- Không đụng: icon/logo, ExtraFee, MoMo, LLMS, Bitbucket, `auth.json` (pre-existing).
- Receipt round 7 theo mẫu deadline; push CHỈ `task/zalopay-postfix-audit`; STOP chờ TL review.
