# QC Test Cases — SL-001 (Hyvä AddressDropdown module) + SL-002 (Launchpad OSC)

> ⚠️ **LEGACY (Phase 1a):** SL-001 portion (sections A–E) superseded by [`records/features/FEAT-001.md`](../records/features/FEAT-001.md) (§Test Summary). SL-002 portion (sections A–D) not yet consolidated. Read-only; excluded from default context loading. Do not edit SL-001 portion — update FEAT-001 instead.
...
> **Status note**: SL-001 customer-form + i18n + cleanup = implemented (testable now). SL-001 cart (AC-005) + resolver (AC-011) = **BLOCKED** (Step 4 / Q1 deferred). SL-002 = **not yet implemented** (forward-looking, contingent on OSC research).
...
# SL-001 — Hyvä-native AddressDropdown (MODULE layer)

## A. Customer address form — cascade (AC-001, BR-002)

### SL-001-TC-001: Tạo address mới — cascade VN 3 cấp
...
### SL-001-TC-002: Edit existing VN address — pre-fill đúng
...
### SL-001-TC-003: Save persist đúng data (Tier 2 — customer data)
...
### SL-001-TC-004: Đổi region → city reset
...
### SL-001-TC-005: GraphQL lỗi — fail gracefully
...
### SL-001-TC-006: Label vi_VN đúng 3 cấp
...
### SL-001-TC-007: Label en_US đúng (VN market)
...
### SL-001-TC-008: Generic module không define storefront label (no conflict)
...
### SL-001-TC-009: vi placeholder KHÔNG phải "Quận/Huyện" (fix district bug)
...
### SL-001-TC-010: No RequireJS/Knockout leak trên storefront (customer form)
...
### SL-001-TC-011: Dead reference Ahamave gone
...
### SL-001-TC-012: Default/Luma checkout code dormant — no leak
...
### SL-001-TC-013: Tailwind classes render (no purge)
...
### SL-001-TC-014: Admin CRUD City/Region/SubCity nguyên vẹn
...
### SL-001-TC-015: ⛔ Cart shipping estimation cascade (AC-005)
...
### SL-001-TC-016: ⛔ Resolver cache / N+1 (AC-011)
- **Status**: BLOCKED — Step 1 (Q1 Tier 2 SA) chưa chốt.
- **When unblocked**: cascade GraphQL có cache; không N+1 trên critical path.

---

# SL-002 — Launchpad OSC Address Integration (forward-looking)

> Precondition khi test: SL-002 đã implement + SL-001 done (module expose interface). OSC = Mageplaza OSC (Knockout qua compat `hyva-themes/magento2-luma-checkout`).

## A. OSC cascade (AC-002, AC-003, BR-002, BR-004)

### SL-002-TC-001: OSC shipping — cascade VN 3 cấp
...
### SL-002-TC-002: OSC billing — cascade + persist
...
### SL-002-TC-003: Edit shipping address trong OSC
...
### SL-002-TC-004: ExtraFee + DeliveryTime không regression
...
### SL-002-TC-005: Mollie payment — place order thành công
...
### SL-002-TC-006: Không coupling OSC trong module (DEC-8)
...
### SL-002-TC-007: Không edit Mageplaza in-place
...
### SL-002-TC-008: OSC i18n vi/en (AC-O3)
...
### SL-002-TC-009: OSC submit mechanism — totals sync đúng
...
### SL-002-TC-010: End-to-end checkout VN address → order
...
## Coverage checklist
- [x] Mỗi AC implemented (SL-001: 001/005/006/007/008/009/010/011/012) có ≥1 TC (005/011 = BLOCKED)
...
## Escalation flags
- **SL-002 TC-009 (OSC seam)**: phụ thuộc research Q2/Q3 (OSC submit mechanism) — chưa chốt → TC có thể thay đổi sau research.
...
- **Cart (SL-001-TC-015) + resolver (TC-016)**: BLOCKED — chạy sau Step 4 + Q1.
