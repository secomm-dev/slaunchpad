# SL-010 — GHTK order/shipment sync (idempotent async outbox, persistence model, business rules)

**Type:** Task (sub-ticket of feature [FEAT-006](../records/features/FEAT-006.md); scope gate)
**Priority:** Medium (sau SL-008/009)
**Estimate:** ~16–24h
...
**Risk tier:** Tier 2
**Author:** AI draft · **Date:** 2026-07-30 · **Status:** 🔁 Redirected 2026-08-17 → [SL-016](SL-016-ghtk-native-label-flow.md) (native shipping-label flow, DEC-SL016-001 supersedes DEC-023 outbound outbox)

> **REDIRECT 2026-08-17:** order submit hiện thực theo **Magento native shipping-label flow** (label-triggered, sync trong label request) — xem SL-016 + DEC-SL016-001. Outbox/cron/webhook inbound của ticket này KHÔNG build; webhook status sync vẫn deferred như cũ. Các business rule DEC-024 được resolve mặc định trong DEC-SL016-001 (B3 is_freeship=1, B8 partner id, B14 trigger=label; B1/B4/B6/B7 defaults; B9 cancel vẫn out-of-phase).



> **PARKED (user 2026-07-30):** order sync KHÔNG hiện thực phase này — chỉ làm phần **fee** (SL-008 + SL-009) trước. Ticket này giữ requirement/design parked để resume sau. DEC-023 (accepted) + DEC-024 (proposed) không block fee delivery. Khi resume: chốt DEC-024 B1/B4/B8/B14 + COD/pick_money tension.

## Description

Đồng bộ Magento shipment → GHTK order (`POST /services/shipment/order`) theo pattern **idempotent async outbox** (DEC-023): KHÔNG gọi GHTK order API bên trong transaction tạo Magento shipment. Ghi sync record trong cùng transaction (DB only) → async cron-backed consumer gọi GHTK → persist trạng thái + label/tracking → retry có kiểm soát + manual retry. Business rules (COD/declared value/freeship/partial/cancel/trigger) pending approval (DEC-024).

## Flow (DEC-023 — accepted 2026-07-30, webhook-first)
...
- [ ] **AC-5b (Inbound webhook status sync — release-1 Core, DEC-023):** GHTK webhook endpoint nhận status change → reconcile idempotent (`partner_order_id`) → update `secomm_ghtk_shipment` + Magento shipment track/status. Endpoint return 200 nhanh; processing sync/cron now (MQ queue deferred). Webhook + outbox share reconcile path.

### Order request building (reuses SL-008/009 resolvers + weight)
- [ ] **AC-6 (Request builder):** build order payload dùng `DestinationAddressResolver` (SL-008) + `PickupAddressResolver` (SL-009) + `ShipmentWeightCalculator` (SL-009). street/hamlet từ address field (DEC-024 B5).

### Business rules (DEC-024 — partial)
...
- [ ] **AC-8 (External contract verify):** order-API field names/types (COD, `pick_money`, `is_freeship`, declared value, label return) verify từ GHTK doc VN/sandbox trước hiện thực mapper (DEC-024 Q-EXT). KHÔNG bịa field/behavior.
...
- **Base URI:** `https://services.giaohangtietkiem.vn`; endpoint `POST /services/shipment/order`. Headers `Token` + `X-Client-Source` (reuse SL-009 client abstraction).
...
- Depends on SL-008 (destination resolver) + SL-009 (pickup resolver, weight, client, mask logger).

## Files/Areas Affected (planned)

- `app/code/Secomm/Ghtk/etc/db_schema.xml` (`secomm_ghtk_shipment`) — NEW (append to SL-008 schema)
...
- Q1–Q14 = DEC-024 B1–B14 (business rules) — **B2/B3 deferred**, **B9 out-of-phase**; **block ready** tối thiểu **B1 (COD) / B4 / B8 / B14**
- Q-EXT (external verify): order-API field contract COD/declared value/label (DEC-024) — still open
...
- [ ] Mask logging (AC-9)
- [ ] DEC-023 (architecture) accepted — webhook-first inbound + outbox outbound hiện thực; DEC-024 business rules: B2/B3 deferred, B9 out-of-phase; tối thiểu **B1 (COD, kèm COD/pick_money tension) / B4 / B8 / B14** approved hoặc pending đúng chuẩn
- [ ] AI pre-review pass
...
- [ ] Evidence `.ai/runtime/evidence/FEAT-006/` (SL-010)

## Related

- Feature: [FEAT-006](../records/features/FEAT-006.md) · Depends on [SL-008](SL-008-ghtk-address-mapping.md) · [SL-009](SL-009-ghtk-carrier-rate.md) · Decisions: [DEC-023](../records/decisions/DEC-023.md) (async/idempotency) · [DEC-024](../records/decisions/DEC-024.md) (business rules) · [DEC-021](../records/decisions/DEC-021.md) · [DEC-022](../records/decisions/DEC-022.md)
