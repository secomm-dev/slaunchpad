# TASK-YJENM2 — GHTK address mapping (table-first, canonical key, hardened CSV upload, best-effort vi_VN fallback)

**Legacy ID:** SL-008 *(re-identified 2026-08-20 per DEC-027; mapping: .ai/toolkit/legacy-id-map.yaml)*
...
**Risk tier:** Tier 2
**Author:** AI draft · **Date:** 2026-07-30 · **Status:** Dev complete *(legacy reconciliation 2026-08-19, DEC-TASKZ132WA-002: shipped trong FEAT-AE761Z — Ghtk address mapping live tại `Secomm/Ghtk/Model/Address`; status "Ready" stale từ 2026-07-30)*

## Description

Tạo **bảng mapping GHTK** (`secomm_ghtk_address_map`) làm **nguồn chính** cho mọi giá trị địa chỉ đưa vào GHTK API. Mọi call API (fee/order ở TASK-BRKHN4/010) đều đi qua **resolver** đọc từ bảng này. Bảng update được qua **CSV upload trong admin config** (hardened). Khi bảng thiếu mapping, resolver **fallback** lấy tên **vi_VN chuẩn** từ data layer để build **best-effort request** (KHÔNG đảm bảo GHTK nhận diện — chỉ đảm bảo có dữ liệu hợp lệ để build request).

### Requirement changes 2026-07-30 (so với bản cũ)

1. **Canonical key đổi** từ `(region_id, ward_name)` → `(country_id, region_id, ward_id)` (DEC-TASKYJENM2-001). `ward_id` = `directory_region_city.city_id` (VN 2-level ward = city level; sub-city là mức 3 cũ, không dùng).
...
4. **Resolver split pointer**: TASK-YJENM2 xây **mapping + destination resolver foundation**; pickup resolver tách (DEC-TASKBRKHN4-001, TASK-BRKHN4).

## Mapping design

**Table `secomm_ghtk_address_map`** (DEC-TASKYJENM2-001):
| Column | Type | Note |
|---|---|---|
| `map_id` | int PK | |
...
> **ward_id availability gap (DEC-TASKYJENM2-001):** ward_id = `directory_region_city.city_id` tồn tại ở DB schema, nhưng GraphQL surface (`GetListCity`) và persistence lưu ward dưới dạng name string → `city_id` không có sẵn trực tiếp trên payload. Path chốt (SA/TL): **(A)** expose+persist `city_id` ở address data layer (generic change, ticket riêng, DEC-FEATJSZQV3-003 boundary) HOẶC **(B)** GHTK-internal name→`city_id` bridge resolver (compatibility/tech-debt tạm). Chọn trước khi TASK-YJENM2 ready.
...
```
Name-normalized fallback chỉ khi request cũ không có `ward_id` (compatibility). Pickup resolver tách riêng (TASK-BRKHN4, DEC-TASKBRKHN4-001).
...
- [ ] **AC-5 (Robustness — fallback semantics đúng):** mapping miss → fallback vi_VN **không throw** (log warning masked); API có thể vẫn reject — caller xử lý graceful (TASK-BRKHN4). **KHÔNG** assert "API luôn nhận giá trị hợp lệ". API không nhận giá trị rỗng cho required field nếu DB có data.
...
- **Reuse data:** fallback vi_VN đọc từ bảng address của `Secomm_AddressDropdown` (VN data do `Secomm_VietNamAddress` import). KHÔNG duplicate master data; KHÔNG sửa generic module bằng carrier-specific mapping (DEC-FEATJSZQV3-003).
...
- **DEC-FEATJSZQV3-003:** bảng mapping + resolver owned bởi `Secomm_Ghtk`; generic module không đổi.
- Phụ thuộc FEAT-JSZQV3: ward đến từ address payload; ward_id gap (DEC-TASKYJENM2-001) phải chốt trước ready.
...
- `app/code/Secomm/Ghtk/etc/adminhtml/system.xml` (upload field trong GHTK section; token/partner ở TASK-BRKHN4)
- **Reuse, không sửa:** `Secomm_AddressDropdown`, `Secomm_VietNamAddress`.

## Risks

- Tier-2: chạm address data + admin upload → escalate (SA/TL).
- **ward_id gap (DEC-TASKYJENM2-001)** — block ready cho đến khi path A/B chốt.
...
- ~~Q1 (SA/TL): ward_id path — (A) expose+persist `city_id` (generic change, ticket riêng) hay (B) GHTK-internal name→id bridge (tech-debt tạm)?~~ **RESOLVED 2026-07-30 (DEC-TASKYJENM2-001 accepted):** path **B** — GHTK-internal name→`city_id` bridge (không sửa generic module; path A = long-term follow-up).
...
- [ ] ward_id path (DEC-TASKYJENM2-001) chốt
- [ ] AI pre-review pass
...
- Spec: [ghtk-address-mapping](../specs/SPEC-TASK-YJENM2-ghtk-address-mapping.md) · Plan: [TASK-YJENM2 plan](../plans/TASK-YJENM2-implementation-plan.md)
- Feature: [FEAT-AE761Z](../records/features/FEAT-AE761Z.md) · Decisions: [DEC-TASKYJENM2-001](../records/decisions/DEC-TASKYJENM2-001.md) (canonical key+fallback) · [DEC-TASKBRKHN4-001](../records/decisions/DEC-TASKBRKHN4-001.md) (resolver split) · [DEC-FEATJSZQV3-002](../records/decisions/DEC-FEATJSZQV3-002.md) · [DEC-FEATJSZQV3-003](../records/decisions/DEC-FEATJSZQV3-003.md)
