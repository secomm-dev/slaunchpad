# SL-012 — Apply VN 2-level address dropdown on admin order forms (create + edit order address)
...
VN = **2 cấp**: `region` (Tỉnh/Thành) → `city` (Phường/Xã = ward); VN adapter không render `sub_city`. **`sub_city` = generic 3rd level (KHÔNG deprecate)** — non-VN 3 cấp vẫn dùng (DEC-025). VN logic ở `Secomm_VietNamAddress`; generic country-agnostic (DEC-019). Architecture: [DEC-025](../records/decisions/DEC-025.md).
...
- [ ] AC-4: **Non-VN country** → Magento default (no cascade); generic (DEC-9/DEC-019).
...
- Admin order create address **không phải ui_component** như customer modal — block-based form (`Magento_Sales`) với JS `mage/backend`, field `order[billing_address][...]` / `order[shipping_address][...]`. → approach khác SL-011 (xem Open Questions).
- Reuse option provider `Model\Customer\Address\Config\Selector\City` (wards by region) — share với SL-011/013/014.
...
- Tuân DEC-019, DEC-8 (boundary — admin order là project/Launchpad hay trong module? → OQ).
...
- Reuse `Model/Customer/Address/Config/Selector/City.php` (share SL-011).
- `Secomm_VietNamAddress/` (label ward — DEC-019).
...
- Order-create JS là native `Magento_Sales` (khác customer modal) → mixin approach ở SL-011 **không port trực tiếp**; approach chưa chốt → cần mini-spec (Mode A).
...
- Q3: Label ward → `Secomm_VietNamAddress` (DEC-019)? — SA.
- Q4: Observer `sub_city` giữ generic (non-VN 3-level, DEC-025) — không gỡ; VN order 2-level không submit. Confirm observer không can thiệp ward (`city`) persist. — verify.

## Dependencies

- [SL-011](SL-011-apply-address-dropdown-admin-customer-form.md) — share `Selector\City` + VN override.
- Coordinate [SL-007](SL-007-generalize-addressdropdown-remove-vn-leak.md) nếu leak VN.
- Liên quan [FEAT-006](../records/features/FEAT-006.md) GHTK — ward đầy đủ ở admin order giúp carrier rate/sync chính xác (không block).
...
- [ ] DEC-019 compliance (no VN leak trong generic)
