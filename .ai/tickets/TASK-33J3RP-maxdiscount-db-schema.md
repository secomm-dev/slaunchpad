# TASK-33J3RP — Data model: column maximum_discount_amount trên salesrule + extension attribute RuleInterface

**Legacy ID:** SL-021 *(re-identified 2026-08-20 per DEC-027; mapping: .ai/toolkit/legacy-id-map.yaml)*

**Type:** Task (slice của FEAT-JKZM68 — data layer)
**Priority:** High
**Estimate:** ~6h
...
**Author:** AI draft · **Date:** 2026-08-19 · **Status:** Dev complete *(2026-08-20: TL Tier-2 approved trong chat → execute Task 1–10 một session. AC-1..AC-5 verified. 2 spec corrections trong lúc verify: AC-1 command sai (`schema:show` không tồn tại), AC-4 semantics thật (disable + upgrade kế tiếp DROP column — document README). Plugin design rework: converter `around` hooks thay vì repository `after` (`Data\Rule` không có public getData). Full `setup:di:compile` từng bị chặn bởi blocker PRE-EXISTING `Secomm_AddressDropdown` × `Secomm_GiaoHangNhanh` (từ 2026-08-11, ngoài scope) — **RESOLVED cùng ngày**: class bị remove kèm PR #20, compile giờ **GREEN** (không cần hotfix); runtime wiring chứng minh bằng service-contract tests xanh. Chờ TL code review. Evidence: [TASK-33J3RP-evidence](../runtime/evidence/TASK-33J3RP/TASK-33J3RP-evidence.md))*
**Specification:** MINI — embedded `## Mini Spec` dưới đây (ID: TASK-33J3RP) · canonical parent: [SPEC-FEAT-JKZM68](../specs/SPEC-FEAT-JKZM68-promotion-max-discount.md) (FULL, VALID) §6 · Plan: [TASK-33J3RP plan](../plans/TASK-33J3RP-implementation-plan.md)

## Description

Implement data model theo DEC-FEATJKZM68-001 §3 (D2 resolved: extension attribute **Phase 1 include**):
...
> Embedded Mini-Spec (DEC-TASKE0SK0H-001 — identity = TASK-33J3RP). Behavioral contract của slice này; business rules canonical thuộc [SPEC-FEAT-JKZM68](../specs/SPEC-FEAT-JKZM68-promotion-max-discount.md) §6, kiến trúc data model thuộc [DEC-FEATJKZM68-001](../records/decisions/DEC-FEATJKZM68-001.md) §3.

### Goal

Triển khai **data layer** cho FEAT-JKZM68: column `maximum_discount_amount` DECIMAL(12,4) UNSIGNED NULL trên bảng core `salesrule` (declarative schema, additive) + extension attribute cho `Magento\SalesRule\Api\Data\RuleInterface` (plugin `RuleRepository` map field) — để TASK-5H8WKE (cap engine) có field đọc được, persist/load đúng qua cả admin lẫn REST, **không PHP glue cho data path native**.
...
- **Totals/pricing không đổi** trong slice này: column inert — chưa có engine nào đọc (TASK-5H8WKE mới consume); mọi cart total, quote, order behave như trước.

### Constraints / Rules

- Thuần **declarative schema** (`db_schema.xml` + `db_schema_whitelist.json` sinh bằng `setup:db-declaration:generate-whitelist`) — cấm InstallSchema/UpgradeSchema/data-patch PHP thủ công; không modify file nào trong `vendor/` (DEC-FEATJKZM68-001, AGENTS §7.1).
- Semantic theo DEC-FEATJKZM68-001 §3: `NULL`/`0` = unlimited (native behavior); `> 0` = cap **base-currency** product discount của rule `by_percent` (đơn vị đồng nhất `by_percent` dùng `discount_amount` làm base — spec §6; runtime guard `simple_action` thuộc TASK-5H8WKE).
...
Cap engine đọc/áp cap (TASK-5H8WKE) · admin form field + validation UI (TASK-67GGPR) · integration tests (TASK-4HYX6Y) · QC matrix + docs (TASK-HPK1WZ) · migration data (không có data cũ — schema trống semantic NULL).
...
- [x] Business rules respected — semantic NULL/0 = unlimited giữ nguyên ở data layer (guard runtime thuộc TASK-5H8WKE)
- [x] Tests — 2 throwaway verify script (model + service contract) ALL GREEN, đã dọn sau khi capture evidence; integration tests thuộc TASK-4HYX6Y
- [x] Performance — plugin chạy mỗi repository read: 1 getData + 1 set, không query thêm; write path chỉ khi ext attr present
- [x] Regression risk: THẤP cho pricing (không collector — column inert tới TASK-5H8WKE); additive API surface: REST response giờ có thêm ext attr (additive, không breaking)
...
- Mini-Spec: embedded `## Mini Spec` (ID TASK-33J3RP) · Parent spec: [SPEC-FEAT-JKZM68](../specs/SPEC-FEAT-JKZM68-promotion-max-discount.md) §6, §14 · Decision: [DEC-FEATJKZM68-001](../records/decisions/DEC-FEATJKZM68-001.md) §3 · Depends: TASK-3R6X8E (scaffold — dev complete)
- Next: TASK-5H8WKE (cap engine consume field) · Plan: [TASK-33J3RP plan](../plans/TASK-33J3RP-implementation-plan.md) (prospective — chờ TL Tier-2 approval để execute)
