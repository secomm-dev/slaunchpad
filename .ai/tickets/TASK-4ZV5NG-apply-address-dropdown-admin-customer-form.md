# TASK-4ZV5NG — Apply/verify VN 2-level address dropdown on admin customer-address form (+ migrate VN logic → VietNamAddress)

**Legacy ID:** SL-011 *(re-identified 2026-08-20 per DEC-027; mapping: .ai/toolkit/legacy-id-map.yaml)*

**Type:** Task (verify + migrate) — DEC-FEATJSZQV3-003 compliance
**Priority:** Medium
**Estimate:** ~6–10h
...
**Spec ref:** [admin-vn-address-customer-form](../specs/SPEC-TASK-4ZV5NG-admin-vn-address-customer-form.md) — **APPROVED 2026-08-03** (user acting as SA/TL; Q1/Q2/Q3 resolved in [plan](../plans/TASK-4ZV5NG-implementation-plan.md)); architecture = [DEC-FEATE2HM1J-001](../records/decisions/DEC-FEATE2HM1J-001.md)
**Risk tier:** Tier 2 (customer data / PII — AGENTS §9 L398, L421)
**Author:** AI draft · **Date:** 2026-08-03 · **Status:** Proposed
**Related:** [TASK-SQY42T](TASK-SQY42T-apply-address-dropdown-admin-order-form.md) · [TASK-8WSERX](TASK-8WSERX-apply-address-dropdown-admin-store-information.md) · [TASK-2V0AEV](TASK-2V0AEV-apply-address-dropdown-admin-msi-source.md)

## Model (DEC-FEATE2HM1J-001 — data-driven levels)

Cascade **data-driven theo country**. VN = **2 cấp**: `region` (Tỉnh/Thành) → `city` (Phường/Xã = ward; `directory_region_city`, DEC-TASKYJENM2-001); VN adapter **không render `sub_city`**. **`sub_city` = generic 3rd level (KHÔNG deprecate)** — country non-VN có data 3 cấp vẫn dùng (DEC-FEATE2HM1J-001). VN logic chỉ ở `Secomm_VietNamAddress`; generic `Secomm_AddressDropdown` country-agnostic (DEC-FEATJSZQV3-003).

## Description

Admin customer-address modal đã wire `city_select` + `sub_city` **trong `Secomm_AddressDropdown`** ([customer_address_form.xml:4](../../app/code/Secomm/AddressDropdown/view/adminhtml/ui_component/customer_address_form.xml#L4); [provider-mixin.js](../../app/code/Secomm/AddressDropdown/view/adminhtml/web/js/form/provider-mixin.js)) — **vi phạm DEC-FEATJSZQV3-003** (VN/level logic leak vào generic module; giống storefront mà TASK-KCBDDT đang dọn).
...
- [ ] AC-4: **Migrate VN logic ra `Secomm_VietNamAddress`** — `Secomm_AddressDropdown` không còn `country=='VN'`/ward-specific behaviour ở admin customer form (DEC-FEATJSZQV3-003/025); generic cascade country-agnostic.
- [ ] AC-5: **`sub_city` generic giữ nguyên** — non-VN country có data 3 cấp vẫn render `sub_city`; VN (2 cấp) không render `sub_city` (data-driven, DEC-FEATE2HM1J-001).
...
- Mirror frontend cart pattern (TASK-FD6A9X `shipping.phtml` + `ValidateVietNamWard`).
...
- Migrate VN logic phải preserve behavior chính xác; không break storefront đang dùng (align TASK-KCBDDT).
...
- Q2: Label ward = "Phường/Xã" qua `Secomm_VietNamAddress` i18n (DEC-FEATJSZQV3-003)? — SA.

## Dependencies

- Share `Selector\City` + ward data với TASK-SQY42T/013/014.
- Coordinate [TASK-KCBDDT](TASK-KCBDDT-generalize-addressdropdown-remove-vn-leak.md) (storefront generic cleanup — cùng boundary).
...
- [ ] DEC-FEATJSZQV3-003/025 compliance (no VN leak trong generic admin surface)
