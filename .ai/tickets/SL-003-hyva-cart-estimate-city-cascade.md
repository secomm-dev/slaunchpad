# SL-003 — Hyva cart "Estimate Shipping & Tax": thêm dropdown City/Sub-city

**Type:** Change Request (slice của feature FEAT-001 — mở rộng AC-005)
**Priority:** Medium
...
- **Placement (DEC-017 — accepted):** code sống trong **`Secomm_VietNamAddress`** (mở rộng `view/frontend`). Override `Magento_Checkout::php-cart/shipping.phtml` **conditional country=VN**, qua layout của `VietNamAddress` (`etc/module.xml` sequence thêm `Secomm_AddressDropdown`). `VietNamAddress` phụ thuộc `AddressDropdown` để reuse data/GraphQL.
...
- Backend extension attribute `sub_city` đã có (FEAT-001) → chỉ cần frontend payload.

## Files/Areas Affected

- `app/code/Secomm/VietNamAddress/view/frontend/` (NEW — layout override `php-cart/shipping.phtml` + templates, conditional country=VN) — per DEC-017.
...
## Resolved decisions (U1–U6, 2026-07-29 — see FEAT-005)

- ~~D1~~ → ANSWERED: VN cần full hierarchy cho mọi shipping method.
- ~~Placement~~ → DEC-017 (extend `Secomm_VietNamAddress`).
- **U1 (ward code)** → per-carrier (DEC-018/019): cart chỉ thu thập admin identifier (ward=`default_name`→native `city`); carrier code ở Launchpad.
...
- **U5 (generic VN-leak)** → ticket riêng [SL-007](SL-007-generalize-addressdropdown-remove-vn-leak.md); không trong task này.
- **U6 (labels)** → DEC-019: generic=Magento default; VN labels ở VietNamAddress.
...
- [ ] FEAT-001 AC-005 cập nhật (region-based → city-based) sau khi Done
