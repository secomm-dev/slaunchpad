# SL-017 — GHTK carrier tracking: webhook primary + Tracking API fallback + normalized status (ShippingCore reusable)
...
**Risk tier:** Tier 2
**Author:** AI draft · **Date:** 2026-08-17 · **Status:** Dev complete *(2026-08-17: Tasks 1-6 done, 144 unit tests green, table created, DI compile OK — chờ TL code review (Tier 2) + QC sandbox webhook; evidence: [SL-017-evidence](../runtime/evidence/SL-017/SL-017-evidence.md))*
...
### GHTK tracking
- [x] **AC-5 (Track đã có):** SL-016 đã persist track (carrier ghtk, number = tracking_code/label_id) — giữ nguyên; processor lookup theo đó.
...
- [x] **AC-14 (Tests):** theo §17: track-created (SL-016 có sẵn — assert processor lookup), webhook valid→update, duplicate no-op, out-of-order giữ DELIVERED, unknown→UNKNOWN+raw, API fallback cùng path, returned/cancelled KHÔNG đụng order. Suite 110+ tests giữ green.
...
- `ORDER_ID_EXIST` reconcile, GHTK label PDF fetch (giữ follow-up từ SL-016).
