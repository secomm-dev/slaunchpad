# SL-022 — Cap engine: MaxDiscountCap collector (sort 310) + CapResolver + LRM allocator

**Type:** Task (slice của FEAT-008 — core engine, phần rủi ro nhất)
**Priority:** High
**Estimate:** ~16–20h
...
**Specification:** SPEC-FEAT-008 (FULL, VALID) — [spec §4 Flow, §5 Hook, §7 Redistribution, §8 Rounding, §9–§11](../specs/SPEC-FEAT-008-promotion-max-discount.md)

## Description

Implement phần lõi của spec — **cap per-rule + redistribution**, theo thuật toán canonical:

1. **`etc/sales.xml`** — quote total collector `max_discount_cap` instance `MaxDiscountCap` **sort_order 310** (sau SalesRule `discount` 300, trước `shipping` 350 — DEC-FEAT008-001 §1).
...
- [ ] **AC-6 (Idempotency):** gọi collect 3 lần liên tiếp với quote không đổi → kết quả identical; unit test hàm pure + integration assert (nếu SL-024 chưa cover thì cover ở đây tối thiểu repeat-collect engine-level).
...
Admin form field (SL-023) · integration/end-to-end tests đầy đủ (SL-024) · cap cho action khác `by_percent` · shipping cap.

## Risks

- **Tier-2 pricing, checkout-critical**: mọi thay đổi totals ảnh hưởng OSC/Mollie/VNPAY payment amount — QC e2e bắt buộc (SL-025); TL code review Tier-2.
...
- Depends: SL-020, SL-021 · Spec: [SPEC-FEAT-008](../specs/SPEC-FEAT-008-promotion-max-discount.md) §4–§11, §13 · Decision: [DEC-FEAT008-001](../records/decisions/DEC-FEAT008-001.md) §1, §2, §4, §6
- Next: SL-023 (∥ SL-024)
