# SL-015 — Secomm ShippingCore origin contract + GHTK origin refactor (shared carrier contract, chuẩn bị fulfillment)
...
**Risk tier:** Tier 2
**Author:** AI draft · **Date:** 2026-08-17 · **Status:** Dev complete *(2026-08-17: Tasks 1-9 done, 84 unit tests green, DI compile OK — chờ TL code review (Tier 2) + QC manual; evidence: [SL-015-evidence](../runtime/evidence/SL-015/SL-015-evidence.md))*
...
- [x] **AC-10 (Rate + shipment cùng origin abstraction):** rate flow dùng provider chain; SL-010 (order sync, parked) khi resume sẽ build ShippingContext từ shipment và gọi cùng `OriginProviderInterface` — ghi rõ trong plan/DEC để tránh origin lệch giữa rate vs submit.
...
### Docs
- [ ] **AC-14 (partial):** README.md + CHANGELOG.md ×2 module DONE; DEC-SL015-001 accepted DONE; còn project-context update (04/09/10) + COMPONENT_INDEX — thực hiện ở closure (sau TL review + QC), theo AGENTS §14.
...
- SL-010 order sync implementation (vẫn parked; chỉ chốt contract cho nó consume).
- Destination resolution (SL-008 machinery) — giữ nguyên.

## Risks

- Tier-2: chạm shipping rate path + shared architecture → escalate SA/TL (DEC-SL015-001 + plan approval).
