# Spec: GHTK Order Submit qua Magento Native Shipping-Label Flow (TASK-KM6YAT)

Specification ID: SPEC-TASK-KM6YAT
Feature ID: NONE
Specification Level: FULL

> **Status:** Implemented — retro-canonical (distilled from approved requirement 2026-08-17 + DEC-TASKKM6YAT-001 + delivery evidence; verified against working tree 2026-08-18).
> **Mode A** · Tier 2 · Tickets: TASK-KM6YAT (redirect TASK-KV328X) · Decision: DEC-TASKKM6YAT-001 (accepted, supersedes DEC-TASKKV328X-001 outbound)
...
Webhook inbound status sync (TASK-86NX9T thực hiện riêng) · GHTK label PDF fetch (dùng generated PDF) · `ORDER_ID_EXIST` auto-reconcile · COD allocation engine / split fulfillment · Ahamove refactor.
...
- BR-LB-06 (Origin): qua TASK-NDASAD chain (`fromShipment()` → provider); shipper contact từ native `Shipment\Request` (admin user + store_information).
- BR-LB-07: Rate calculation độc lập shipment creation — dùng GHTK rate ≠ tạo GHTK order.

# System Behaviour

- `pick_money` = resolved COD; `is_freeship = 1`; `value` = subtotal item trong shipment; weight = Σ package weights (fallback product weights, gram — DEC-TASKBRKHN4-002); partner ORDER_ID = `ghtk-{order_increment}-{seq}` (seq = shipments đã save + 1 — deterministic, shipment-id-independent).
...
AC-1..15 theo [ticket TASK-KM6YAT](../tickets/TASK-KM6YAT-ghtk-native-label-flow.md) (native carrier online; normal-no-API; submit đúng payload; no false success; native persistence; origin qua provider; COD resolver; is_freeship; partial fail-fast; partner id; tests; config; docs). Evidence: `.ai/runtime/evidence/TASK-KM6YAT/`.

# Related Decisions

DEC-TASKKM6YAT-001 (accepted — supersedes DEC-TASKKV328X-001 outbound) · DEC-TASKNDASAD-001 · DEC-TASKBRKHN4-002 (weight) · DEC-TASKKV328X-002 partial-resolve (B1/B3/B4/B6/B7/B8/B14).
