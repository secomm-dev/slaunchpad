# Feature Spec — Admin VN 2-level address dropdown trên Store Information + Shipping Origin config (TASK-8WSERX)

Specification ID: SPEC-TASK-8WSERX
Feature ID: NONE
Specification Level: FULL

<!-- Generated for Secomm Launchpad · Stack: Magento 2.4.8-p5 + Hyvä 3.x -->
<!-- Spec cho ticket TASK-8WSERX (merged: Store Information + Shipping Origin). Parent: FEAT-E2HM1J. Mode B · Tier-1 (admin config; reversible). -->
<!-- Decisions: DEC-FEATE2HM1J-001 (global mechanism + country adapter; sub_city generic 3rd level) · DEC-TASKYJENM2-001 (ward = city level, name-string persistence) · DEC-FEATJSZQV3-003 (generic Magento-default; VN → VietNamAddress) · DEC-17 (VN capability lives in VietNamAddress). -->
<!-- Note: hai surface này là `system.xml` config form (render bởi `Magento_Config`), KHÔNG phải `ui_component` (TASK-4ZV5NG) hay block-form `Magento_Sales` (TASK-SQY42T); cơ chế inject = `frontend_model` trên field `city` của core config. -->
...
**Ticket reference**: [TASK-8WSERX](../tickets/TASK-8WSERX-apply-address-dropdown-admin-store-information.md) · Parent [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md) · extends CMP-ADDR [FEAT-YVN39K](../records/features/FEAT-YVN39K.md)

**Feature type**: New admin config surface (apply mechanism đã pin ở DEC-FEATE2HM1J-001 lên config form)

**Priority**: P3 (Medium — một trong bốn surface admin của FEAT-E2HM1J; Tier-1)
...
FEAT-E2HM1J pin architecture cho mọi admin address surface. Spec (mini-spec — Mode B) này chỉ bao phủ **TASK-8WSERX**: hai config surface admin — **Store Information** (`Stores → Configuration → General → Store Information`, path `general/store_information`) và **Shipping Origin** (`Stores → Configuration → Sales → Shipping Settings → Origin`, path `shipping/origin`). Các surface khác có spec riêng: customer address (TASK-4ZV5NG), order create/edit (TASK-SQY42T), MSI Source (TASK-2V0AEV).
...
- **US-002**: As a developer/maintainer, tôi muốn cơ chế inject dropdown vào config field `city` là generic, country-agnostic (`Secomm_AddressDropdown`) còn behaviour/label/validate VN cô lập trong `Secomm_VietNamAddress`, để tuân DEC-FEATJSZQV3-003/025 và reusable cho surface khác.
...
> Mỗi AC = một test case. AC-1..AC-7 bám theo ticket TASK-8WSERX.

- [ ] **AC-1 (Store Information — VN):** Trên form `Stores → Configuration → General → Store Information`, chọn country = VN → cascade region (Tỉnh/Thành) → `city` (Phường/Xã = ward) render: ward dropdown phụ thuộc region; ward options theo `region_id` (reuse `GetListCity(area=adminhtml)`); chọn ward → điền ward name vào value carrier `city` field. Store Information **không render `sub_city`** cho VN (2-level, DEC-FEATE2HM1J-001).
...
- [ ] **AC-4 (Non-VN — no VN leak):** Non-VN country → Magento default (region updater mặc định + `city` text input; `sub_city` chỉ hiện nếu country có data 3 cấp — DEC-FEATE2HM1J-001); không có behaviour/label VN rò rỉ; không ép ward dropdown.
...
### Cơ chế config form (khác TASK-4ZV5NG/TASK-SQY42T)

Cả 2 surface là **`system.xml` config form** render bởi `Magento_Config` (`Magento\Config\Block\System\Config\Form`), **không phải** `ui_component` (TASK-4ZV5NG customer modal) hay block-form `Magento_Sales` (TASK-SQY42T order). Cấu trúc field của cả 2 nhóm giống hệt (verified 2026-08-07):
...
→ **Approach injection** = override field `city` của core config qua `system.xml` của `Secomm_AddressDropdown` với `frontend_model` (Magento merge attribute `frontend_model` vào field core khi cùng path). `frontend_model` render: native `<input>` (giữ làm **value carrier** — mang đúng config field name `groups[...][fields][city][value]` để Magento persist) + `<select>` ward (`city_select`) + `<select>` `sub_city` generic + `data-mage-init` config wire vào cascade JS generic (mirror precedent TASK-SQY42T `Renderer/City.php` + `city.phtml`).

### Reuse generic mechanism (DEC-FEATE2HM1J-001)
...
- **Cascade JS (generic, country-agnostic):** precedent [`address-city.js`](../../app/code/Secomm/AddressDropdown/view/adminhtml/web/js/order/address-city.js) (TASK-SQY42T) dùng suffix-selector `[id$="_country_id"]` / `[id$="_region_id"]` / `[id$="_region"]` + hydration watcher (poll `country|region_id` ~5min cho Prototype `regionUpdater` async) + GraphQL `GetListCity(area=adminhtml)` + stale guard. **Field-id config form cũng kết thúc `_country_id`/`_region_id`/`_city`** → generic JS tương thích config form theo thiết kế → ưu tiên **reuse / factor** thay vì clone.
- **`sub_city` = generic 3rd level** (DEC-FEATE2HM1J-001, không deprecate): country non-VN có data 3 cấp → `sub_city` hiện (data-driven); VN 2 cấp → không render `sub_city` cho VN.

### VN adapter (DEC-FEATJSZQV3-003)

`Secomm_VietNamAddress` owns (cho config surface):
- Khi `country == 'VN'` → `city` = ward; label ward = "Phường/Xã" (i18n VietNamAddress, đã có từ TASK-4ZV5NG); 2-level → không render `sub_city`.
...
> **Note (data-driven ownership):** như TASK-4ZV5NG deviation đã kết luận, generic mechanism có thể **tự** xử lý VN 2-level tự nhiên (GetListSubCity trả empty cho VN → `sub_city` tự hide). VN-owned logic thực sự cần = server-side validate. Label explicit VN ("Phường/Xã") = optional VN client overlay; decide trong plan.

### Persist

- `core_config_data.value` cho `{path}/city` = ward name string (VN — tech-debt DEC-TASKYJENM2-001 name-string, path A = future); `region_id` đã persist native. **Không** schema migration, **không** cross-wire store.
...
- **NEW** `Secomm_VietNamAddress/Observer/Adminhtml/Config/ValidateVietNamWard.php` (hoặc plugin `Magento\Config\Model\Config::save`) — validate ward ∈ province khi VN; neutralise + log + non-fatal (mirror TASK-4ZV5NG customer plugin).
- `Secomm_VietNamAddress/i18n/vi_VN.csv` + `en_US.csv` — "Phường/Xã" / "Ward/Commune" (đã có từ TASK-4ZV5NG — verify).
- **KHÔNG affect**: storefront `view/frontend/*`, customer/order address admin surface (TASK-4ZV5NG/012), MSI (TASK-2V0AEV), `db_schema`, GraphQL resolvers (reuse read-only).
...
| [DEC-FEATJSZQV3-003](../records/decisions/DEC-FEATJSZQV3-003.md) + DEC-17 (generic vs VN boundary) | decision | accepted | generic config field override; VN validate/label ở VietNamAddress |
| [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md) | feature | proposed | Parent architecture cho TASK-4ZV5NG…TASK-2V0AEV |
| [TASK-4ZV5NG](../tickets/TASK-4ZV5NG-apply-address-dropdown-admin-customer-form.md) | ticket | proposed/in-progress | Share `CityLocaleCollection` + validate pattern (`ValidateVietNamWard`) + i18n label |
| [TASK-SQY42T](../tickets/TASK-SQY42T-apply-address-dropdown-admin-order-form.md) | ticket | in-progress | Share generic cascade JS pattern (`address-city.js` suffix-selector) + Renderer/City.php precedent |
| [TASK-2V0AEV](../tickets/TASK-2V0AEV-apply-address-dropdown-admin-msi-source.md) | ticket | proposed | Share ward source/label logic |
| [FEAT-AE761Z](../records/features/FEAT-AE761Z.md) GHTK | feature | proposed | Shipping Origin = carrier origin/pickup parity (không block) |
...
| **Region updater mặc định** (Prototype `regionUpdater`) xung đột cascade JS | M | M | Reuse hydration watcher `(country\|region_id)` của `address-city.js` (TASK-SQY42T đã proven cho admin region async); QC country switch + region change |
...
| Server-side validate hook sai config save path → không bắt ward sai / chặn save hợp lệ | M | M | Scope 2 path chính xác + gate VN + neutralise (không throw) + non-fatal catch; mirror TASK-4ZV5NG plugin; QC ward sai → neutralise + log, save ok |
...
- Các admin surface khác: customer address (TASK-4ZV5NG), order create/edit (TASK-SQY42T), MSI Source (TASK-2V0AEV).
...
- Evidence → `.ai/runtime/evidence/FEAT-E2HM1J/` (TASK-8WSERX). [ASSUMPTION: evidence path theo convention FEAT-E2HM1J; confirm nếu project dùng path khác.]
...
- **Q1 (approach — chốt trong plan):** Override core config field `city` qua `system.xml` `frontend_model` (Magento-idiomatic, merge attribute) **hay** plugin vào `Magento\Config\Block\System\Config\Form` render? → **khuyến nghị frontend_model** (cleanest, low-risk, mirror TASK-SQY42T Renderer precedent). Chốt plan.
- **Q2 (ward source cho config — chốt trong plan):** Static `source_model` hay dynamic GraphQL? → Static `source_model` **không filter được theo region** (config source load 1 lần); → **khuyến nghị reuse GraphQL `GetListCity(area=adminhtml)` + generic cascade JS** (như TASK-SQY42T); cho server validate reuse `CityLocaleCollection` read-only. Chốt plan.
- **Q3 (label ownership):** Label "Phường/Xã" cho VN = generic renders từ data (implicit) hay VN explicit client overlay (như TASK-4ZV5NG deviation để mở)? → decide plan (ưu tiên implicit data-driven nếu đủ; explicit VN overlay nếu TL muốn ownership rõ).
