# Feature Spec — Admin VN 2-level address dropdown trên order form (TASK-SQY42T)

Specification ID: SPEC-TASK-SQY42T
Feature ID: NONE
Specification Level: FULL

<!-- Generated for Secomm Launchpad · Stack: Magento 2.4.8-p5 + Hyvä 3.x -->
<!-- Spec cho ticket TASK-SQY42T (create + edit order address). Parent: FEAT-E2HM1J. Mode A · Tier-2 (order/PII). -->
<!-- Decisions: DEC-FEATE2HM1J-001 (global mechanism + country adapter; sub_city generic 3rd level) · DEC-TASKYJENM2-001 (ward = city level, name-string persistence tech-debt) · DEC-FEATJSZQV3-003 (generic Magento-default; VN → VietNamAddress) · DEC-17 (VN capability lives in VietNamAddress). -->
...
**Ticket reference**: [TASK-SQY42T](../tickets/TASK-SQY42T-apply-address-dropdown-admin-order-form.md) · Parent [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md) · extends CMP-ADDR [FEAT-YVN39K](../records/features/FEAT-YVN39K.md)
...
FEAT-E2HM1J pin architecture cho mọi admin address surface. Spec này chỉ bao phủ **TASK-SQY42T**: admin **order create** (billing + shipping) và **order address edit**. Các surface khác có spec riêng: customer address (TASK-4ZV5NG), Store Information + Shipping Origin (TASK-8WSERX), MSI Source (TASK-2V0AEV).
...
- **US-003**: As a maintainer, I want VN behaviour isolated in `Secomm_VietNamAddress` and the generic cascade left country-agnostic so that the order form follows DEC-FEATJSZQV3-003/025 and does not leak VN logic into the generic module.
...
- Admin order create/edit là **block-based `Magento_Sales` flow**. Đây không phải `ui_component` giống customer address modal, nên không thể port 1:1 các hook của TASK-4ZV5NG.
...
- `Secomm_VietNamAddress` nên sở hữu label VN và ward behavior khi `country == 'VN'`; generic `Secomm_AddressDropdown` giữ cascade country-agnostic theo DEC-FEATE2HM1J-001.
...
|------------|------|--------|-------|
| [DEC-FEATE2HM1J-001](../records/decisions/DEC-FEATE2HM1J-001.md) | decision | accepted | Global mechanism + country adapter; `sub_city` generic |
| [DEC-TASKYJENM2-001](../records/decisions/DEC-TASKYJENM2-001.md) | decision | accepted | Ward = city level; name-string persistence tech-debt |
| [DEC-FEATJSZQV3-003](../records/decisions/DEC-FEATJSZQV3-003.md) + DEC-17 | decision | accepted | Generic vs VN boundary |
| [FEAT-E2HM1J](../records/features/FEAT-E2HM1J.md) | feature | proposed | Parent architecture cho TASK-4ZV5NG…TASK-2V0AEV |
| [TASK-4ZV5NG](../tickets/TASK-4ZV5NG-apply-address-dropdown-admin-customer-form.md) | ticket | proposed | Shared selector + VN adapter pattern |
| [TASK-8WSERX](../tickets/TASK-8WSERX-apply-address-dropdown-admin-store-information.md) | ticket | proposed | Shared ward source/label logic |
| [TASK-2V0AEV](../tickets/TASK-2V0AEV-apply-address-dropdown-admin-msi-source.md) | ticket | proposed | Shared ward source/label logic |
| [TASK-FD6A9X](./vn-cart-shipping-estimate.md) / [FEAT-JSZQV3](../records/features/FEAT-JSZQV3.md) | feature | proposed | Precedent cho `ValidateVietNamWard` + ward dropdown behavior |
...
- Customer-address admin flow (`TASK-4ZV5NG`).
- Store Information + Shipping Origin config (`TASK-8WSERX`).
- MSI Source form (`TASK-2V0AEV`).
- Storefront generic cleanup / VN leak cleanup (`TASK-KCBDDT`).
- Canonical `ward_id` / `city_id` persistence path A (DEC-TASKYJENM2-001 long-term follow-up).
