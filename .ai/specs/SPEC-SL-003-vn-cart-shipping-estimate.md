...
| Date | 2026-07-29 |
| Related Ticket(s) | [SL-003](../tickets/SL-003-hyva-cart-estimate-city-cascade.md) |
| Parent Feature | [FEAT-005](../records/features/FEAT-005.md) |
| Workflow Mode | A (Tier-2: shipping estimate + address data) |
| Decisions | DEC-017 (generic→VietNamAddress) · DEC-018 (per-carrier→Launchpad) · DEC-019 (label/code ownership) · DEC-8 (boundary) |
...
- **Carrier code mapping** (GHN/Ahamove/TableRate codes) → Launchpad per-carrier (SL-004/005/006, DEC-018). Cart chỉ thu thập admin identifier.
- **Mageplaza OSC** (trừ shared address logic reuse không mở scope).
- **Generic VN-leak cleanup** → SL-007 (DEC-019).
- Schema/extension_attribute/GraphQL **mới**; resolver hardening (FEAT-001 AC-011/Q1).
...
- Mở rộng `Secomm_VietNamAddress` (data-only → +`view/frontend` cart adapter + validation). **Không đổi** `Secomm_AddressDropdown` (generic, country-agnostic — DEC-019).
...
- Mapping (DEC-019): Province = Magento `region_id`/`region`; Ward = `default_name` → native `city`. **Không** carrier code trong cart.
...
**None.** VN 2-level data đã import ([VN_Address_2Level.csv](../../app/code/Secomm/VietNamAddress/Files/VN_Address_2Level.csv) → `directory_country_region` + `directory_region_city`). Không district. Không storage/code mới (DEC-019: ward = `default_name`).
...
Shipping carriers consume province + ward qua `RateRequest` ([Plugin/Model/Shipping.php:57-107](../../app/code/Secomm/AddressDropdown/Plugin/Model/Shipping.php#L57-L107) đã mutate `dest_city`/`sub_city`). **Carrier code mapping = Launchpad** (DEC-018, out of scope).
