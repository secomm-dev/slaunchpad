# Spec: Secomm ShippingCore Origin Contract (TASK-NDASAD)

Specification ID: SPEC-TASK-NDASAD
Feature ID: NONE
Specification Level: FULL

> **Status:** Implemented — retro-canonical (distilled from approved requirement 2026-08-17 + DEC-TASKNDASAD-001 + delivery evidence; verified against working tree 2026-08-18).
> **Mode A** · Tier 2 · Tickets: TASK-NDASAD · Decision: DEC-TASKNDASAD-001 (accepted)
...
MSI source-based origin / nearest store / inventory routing / split fulfillment / Pancake / OMS (`Secomm_ShippingFulfillment` — ticket kế) · refactor `Secomm_Ahamove` · GHN module · TASK-KV328X order sync · destination resolution (TASK-YJENM2 giữ nguyên).
...
- BR-SC-03 (BC GHTK): legacy `pick_*` set (bất kỳ field) thắng; TẤT CẢ trống mới fallback Shipping Origin; half-filled KHÔNG fallback im lặng (strict DEC-TASKBRKHN4-001 — tránh ship từ kho sai).
...
- `Origin` = immutable VO: sourceCode, countryId, regionId (giữ nguyên để carrier normalize tên bằng id — DEC-TASKYJENM2-001), province, **district (nullable — Launchpad VN model)**, ward, street, postcode, telephone, contactName, metadata.
- Provider trả **data snapshot**, KHÔNG quyết validity — usability là policy của carrier (DEC-TASKBRKHN4-001 gate nằm trong carrier).
...
2. Label submit (TASK-KM6YAT): `Shipment → fromShipment() → cùng chain`.
...
(Chi tiết: [ticket TASK-NDASAD](../tickets/TASK-NDASAD-shippingcore-origin-contract.md); evidence: `.ai/runtime/evidence/TASK-NDASAD/`)

# Related Decisions

DEC-TASKNDASAD-001 (accepted) · DEC-TASKYJENM2-001/021/022 · DEC-FEATJSZQV3-002/019 · TASK-8WSERX/TASK-2V0AEV (admin VN origin data).
