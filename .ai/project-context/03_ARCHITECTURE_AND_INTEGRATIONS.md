...
| Pancake POS | fulfillment (offline OMS) | bidirectional | REST/HTTP (HTTPS) | high | query-param `api_key` (ApiKeyAuth) | SLP-30: `Secomm_FulfillmentCore` + `Secomm_PancakeBridge` (Magento wiring) + `Secomm_PancakeFunction` (POS HTTP/payload/mapper). Legacy `Secomm_Pancake` deprecated. Contract: [05 §Pancake POS](05_API_CONTRACTS.md). |
...
- **Order → Fulfillment (Pancake POS)** *(SLP-30)*: Core orchestrates export/inbound; Bridge wires Magento (cron/webhook/admin); Function calls POS HTTP. Outbound create (`custom_id` = `increment_id`); inbound poll + optional webhook. Contract: [05 §Pancake POS](05_API_CONTRACTS.md).
